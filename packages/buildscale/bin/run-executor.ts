...
if (process.env.BUILDSCALE_TERMINAL_OUTPUT_PATH) {
  setUpOutputWatching(
    process.env.BUILDSCALE_TERMINAL_CAPTURE_STDERR === 'true',
    process.env.BUILDSCALE_STREAM_OUTPUT === 'true'
  );
}

if (!process.env.BUILDSCALE_WORKSPACE_ROOT) {
  console.error('Invalid Buildscale command invocation');
  process.exit(1);
}
...
function setUpOutputWatching(captureStderr: boolean, streamOutput: boolean) {
  ...
  const outputPath = process.env.BUILDSCALE_TERMINAL_OUTPUT_PATH;
  ...
  async (message: {
    ...
    try {
      const statusCode = await run(
        process.cwd(),
        process.env.BUILDSCALE_WORKSPACE_ROOT,
