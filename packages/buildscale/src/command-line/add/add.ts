...
async function installPackage(pkgName: string, version: string): Promise<void> {
  ...
  if (existsSync('package.json')) {
    ...
    buildscaleJson.installation.plugins[pkgName] = version;
...
function parsePackageSpecifier(
  ...
  if (i <= 0) {
    if (coreBuildscalePlugins.includes(packageSpecifier)) {
      return [packageSpecifier, buildscaleVersion];
    }
