...
export function withRunOptions<T>(yargs: Argv<T>): Argv<T & RunOptions> {
  return withExcludeOption(yargs)
    ...
    .options('runner', {
      describe: 'This is the name of the tasks runner configured in buildscale.json',
      type: 'string',
    })
    .option('prod', {
...
export function withBatch(yargs: Argv) {
  return yargs.options('batch', {
    ...
    coerce: (v) => {
      return v || process.env.BUILDSCALE_BATCH_MODE === 'true';
    },
    default: false,
  }) as any;
}
...
export function withAffectedOptions(yargs: Argv) {
  return withExcludeOption(yargs)
    ...
    .option('files', {
      describe:
        'Change the way Buildscale is calculating the affected command by providing directly changed files, list of files delimited by commas or spaces',
      type: 'string',
      requiresArg: true,
      coerce: parseCSV,
    })
...
export function withOutputStyleOption(
  ...
  return yargs.option('output-style', {
    describe: 'Defines how Buildscale emits outputs tasks logs',
    type: 'string',
    choices,
  });
}
...
export function withRunOneOptions(yargs: Argv) {
  ...
  if (executorShouldShowHelp) {
    ...
    return res.epilog(
      `Run "buildscale run myapp:mytarget --help" to see information about the executor's schema.`
    );
  }
}
