...
import { yargsBuildscaleInfixCommand, yargsRunCommand } from './run/command-object';
...
export const commandsObject = yargs
  ...
  .command(yargsBuildscaleInfixCommand)
  .scriptName('buildscale')
  .help()
  // NOTE: we handle --version in.buildscalew.ts, this just tells yargs that the option exists
  // so that it shows up in help. The default yargs implementation of --version is not
  // hit, as the implementation in.buildscalew.ts is hit first and calls process.exit(0).
  .version();
