...
export async function resolveBuildscaleJsonConfigErrorMessage(
  propPath: string[]
): Promise<string> {
  const errorLines = await getJsonConfigLinesForErrorMessage(
    readFileSync(joinPathFragments(workspaceRoot, 'buildscale.json'), 'utf-8'),
    propPath
  );
  let buildscaleJsonMessage = `The relevant config is defined here: ${relative(
    process.cwd(),
    joinPathFragments(workspaceRoot, 'buildscale.json')
  )}`;
  if (errorLines) {
    buildscaleJsonMessage +=
      errorLines.startLine === errorLines.endLine
        ? `, line ${errorLines.startLine}`
        : `, lines ${errorLines.startLine}-${errorLines.endLine}`;
  }
  return buildscaleJsonMessage;
}
