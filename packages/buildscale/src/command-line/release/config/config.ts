/**
 * `buildscale release` is a powerful feature which spans many possible use cases. The possible variations
 * of configuration are therefore quite complex, particularly when you consider release groups.
 *
 * We want to provide the best possible DX for users so that they can harness the power of `buildscale release`
 * most effectively, therefore we need to both provide sensible defaults for common scenarios (to avoid
 * verbose buildscale.json files wherever possible), and proactively handle potential sources of config issues
 * in more complex use-cases.
 *
 * This file is the source of truth for all `buildscale release` configuration reconciliation, including sensible
 * defaults and user overrides, as well as handling common errors, up front to produce a single, consistent,
 * and easy to consume config object for all the `buildscale release` command implementations.
 */
 ...
 */
export type BuildscaleReleaseConfig = Omit<
  DeepRequired<
    BuildscaleJsonConfiguration['release'] & {
      groups: DeepRequired<
        RemoveTrueFromPropertiesOnEach<
          EnsureProjectsArray<BuildscaleJsonConfiguration['release']['groups']>,
          'changelog'
        >
      >;
      ...
      changelog: RemoveTrueFromProperties<
        DeepRequired<BuildscaleJsonConfiguration['release']['changelog']>,
        ...
        types: RemoveBooleanFromPropertiesOnEach<
          DeepRequired<
            RemoveBooleanFromProperties<
              DeepRequired<
                BuildscaleJsonConfiguration['release']['conventionalCommits']['types']
              >,
              string
            >
          >,
...
export interface CreateBuildscaleReleaseConfigError {
  code:
...
export async function createBuildscaleReleaseConfig(
  projectGraph: ProjectGraph,
  projectFileMap: ProjectFileMap,
  userConfig: BuildscaleJsonConfiguration['release'] = {}
): Promise<{
  error: null | CreateBuildscaleReleaseConfigError;
  buildscaleReleaseConfig: BuildscaleReleaseConfig | null;
}> {
  ...
  const WORKSPACE_DEFAULTS: Omit<BuildscaleReleaseConfig, 'groups'> = {
  ...
  const GROUP_DEFAULTS: Omit<BuildscaleReleaseConfig['groups'][string], 'projects'> = {
   ...
   */
  const rootGitConfig: BuildscaleReleaseConfig['git'] = deepMergeDefaults(
    [WORKSPACE_DEFAULTS.git],
    userConfig.git as Partial<BuildscaleReleaseConfig['git']>
  );
  const rootVersionConfig: BuildscaleReleaseConfig['version'] = deepMergeDefaults(
    [
      WORKSPACE_DEFAULTS.version,
      // Merge in the git defaults from the top level
      { git: versionGitDefaults } as BuildscaleReleaseConfig['version'],
      {
        git: userConfig.git as Partial<BuildscaleReleaseConfig['git']>,
      } as BuildscaleReleaseConfig['version'],
    ],
    userConfig.version as Partial<BuildscaleReleaseConfig['version']>
  );
  ...
  const rootChangelogConfig: BuildscaleReleaseConfig['changelog'] = deepMergeDefaults(
    [
      WORKSPACE_DEFAULTS.changelog,
      // Merge in the git defaults from the top level
      { git: changelogGitDefaults } as BuildscaleReleaseConfig['changelog'],
      {
        git: userConfig.git as Partial<BuildscaleReleaseConfig['git']>,
      } as BuildscaleReleaseConfig['changelog'],
    ],
    normalizeTrueToEmptyObject(userConfig.changelog) as Partial<
      BuildscaleReleaseConfig['changelog']
    >
  );

  const rootConventionalCommitsConfig: BuildscaleReleaseConfig['conventionalCommits'] =
    deepMergeDefaults(
      ...
      fillUnspecifiedConventionalCommitsProperties(
        ...
        ) as BuildscaleReleaseConfig['conventionalCommits']
      )
    );
  ...
  const groups: BuildscaleReleaseConfig['groups'] =
   ...
   */
  const releaseGroups: BuildscaleReleaseConfig['groups'] = {};
  ...
  for (const [releaseGroupName, releaseGroup] of Object.entries(groups)) {
    ...
    const groupChangelogDefaults: Array<
      BuildscaleReleaseConfig['groups']['string']['changelog']
    ...
    if (releaseGroup.changelog) {
      ...
      ) as BuildscaleReleaseConfig['groups']['string']['changelog'];
    }

    const groupDefaults: BuildscaleReleaseConfig['groups']['string'] = {
      ...
      changelog:
        releaseGroup.changelog || rootChangelogConfig.projectChangelogs
          ? deepMergeDefaults<BuildscaleReleaseConfig['groups']['string']['changelog']>(
...
function normalizeConventionalCommitsConfig(
  userConventionalCommitsConfig: BuildscaleJsonConfiguration['release']['conventionalCommits']
): BuildscaleJsonConfiguration['release']['conventionalCommits'] {
  if (!userConventionalCommitsConfig || !userConventionalCommitsConfig.types) {
    return userConventionalCommitsConfig;
  }

  const types: BuildscaleJsonConfiguration['release']['conventionalCommits']['types'] =
    {};
...
function fillUnspecifiedConventionalCommitsProperties(
  config: BuildscaleReleaseConfig['conventionalCommits']
) {
  if (!config || !config.types) {
    return config;
  }
  const types: BuildscaleReleaseConfig['conventionalCommits']['types'] = {};
...
export async function handleBuildscaleReleaseConfigError(
  error: CreateBuildscaleReleaseConfigError
): Promise<never> {
  switch (error.code) {
...
): null | CreateBuildscaleReleaseConfigError {
...
function ensureProjectsConfigIsArray(
  groups: BuildscaleJsonConfiguration['release']['groups']
): BuildscaleReleaseConfig['groups'] {
  const result: BuildscaleJsonConfiguration['release']['groups'] = {};
  ...
  return result as BuildscaleReleaseConfig['groups'];
}
 ...
 * global configuration if using the top level buildscale release command and the granular configuration if using
