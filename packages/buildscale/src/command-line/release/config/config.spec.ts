...
import { createBuildscaleReleaseConfig } from './config';
...
describe('createBuildscaleReleaseConfig()', () => {
  let projectGraph: ProjectGraph;
  let projectFileMap: ProjectFileMap;
  let tempFs: TempFs;

  beforeEach(async () => {
    tempFs = new TempFs('buildscale-release-config-test');
    await tempFs.createFiles({
      ...
      'packages/buildscale/package.json': JSON.stringify({
        name: 'buildscale',
        version: '0.0.0',
      }),
    });
    ...
    projectFileMap = {
      ...
      buildscale: [
        {
          file: 'packages/buildscale/package.json',
          hash: 'abc',
        },
      ],
      root: [
  ...
  describe('zero/empty user config', () => {
    it('should create appropriate default BuildscaleReleaseConfig data from zero/empty user config', async () => {
      // zero user config
      expect(
        await createBuildscaleReleaseConfig(projectGraph, projectFileMap, undefined)
      ...
      // empty user config
      expect(await createBuildscaleReleaseConfig(projectGraph, projectFileMap, {}))
        .toMatchInlineSnapshot(`
        {
          "error": null,
          "buildscaleReleaseConfig": {
      ...
      // empty groups
      expect(
        await createBuildscaleReleaseConfig(projectGraph, projectFileMap, {
          groups: {},
        })
      ).toMatchInlineSnapshot(`
        {
  ...
  describe('user specified groups', () => {
    it('should ignore any projects not matched to user specified groups', async () => {
      const res = await createBuildscaleReleaseConfig(projectGraph, projectFileMap, {
        groups: {
          'group-1': {
  ...
  describe('user config -> top level version', () => {
    ...
    it('should allow configuration of preVersionCommand', async () => {
      const res = await createBuildscaleReleaseConfig(projectGraph, projectFileMap, {
        version: {
          preVersionCommand: 'buildscale run-many -t build',
        },
      });
      expect(res).toMatchInlineSnapshot(`
        {
  ...
  describe('user config -> top level conventional commits configuration', () => {
    it('should use defaults when config is empty', async () => {
      const res1 = await createBuildscaleReleaseConfig(projectGraph, projectFileMap, {
        conventionalCommits: {},
      });
      ...
      const res2 = await createBuildscaleReleaseConfig(projectGraph, projectFileMap, {
        conventionalCommits: {
          types: {},
        },
      });
