...
export interface BuildscaleReleaseVersionResult {
  /**
...
export async function releaseVersion(
  args: VersionOptions
): Promise<BuildscaleReleaseVersionResult> {
  ...
  if (
    ...
    output.error({
      title: `The "release.git" property in buildscale.json may not be used with the "buildscale release version" subcommand or programmatic API. Instead, configure git options for subcommands directly with "release.version.git" and "release.changelog.git".`,
  ...
  runPreVersionCommand(buildscaleReleaseConfig.version.preVersionCommand, {
    dryRun: args.dryRun,
  ...
  const commitMessage: string | undefined =
    args.gitCommitMessage || buildscaleReleaseConfig.version.git.commitMessage;
  ...
  if (args.projects?.length) {
    ...
    const gitTagValues: string[] =
      args.gitTag ?? buildscaleReleaseConfig.version.git.tag
        ? createGitTagValues(
            releaseGroups,
            releaseGroupToFilteredProjects,
            versionData
    ...
    if (args.gitCommit ?? buildscaleReleaseConfig.version.git.commit) {
      await commitChanges(
        changedFiles,
        !!args.dryRun,
        !!args.verbose,
        ...
        args.gitCommitArgs || buildscaleReleaseConfig.version.git.commitArgs
      );
    } else if (args.stageChanges ?? buildscaleReleaseConfig.version.git.stageChanges) {
    ...
    if (args.gitTag ?? buildscaleReleaseConfig.version.git.tag) {
      output.logSingleLine(`Tagging commit with git`);
      for (const tag of gitTagValues) {
        await gitTag({
          tag,
          message: args.gitTagMessage || buildscaleReleaseConfig.version.git.tagMessage,
          additionalArgs:
            args.gitTagArgs || buildscaleReleaseConfig.version.git.tagArgs,
          dryRun: args.dryRun,
          verbose: args.verbose,
        });
      }
  ...
  if (args.gitTag ?? buildscaleReleaseConfig.version.git.tag) {
    ...
    for (const tag of gitTagValues) {
      await gitTag({
        tag,
        message: args.gitTagMessage || buildscaleReleaseConfig.version.git.tagMessage,
        additionalArgs: args.gitTagArgs || buildscaleReleaseConfig.version.git.tagArgs,
        dryRun: args.dryRun,
        verbose: args.verbose,
      });
    }
...
interface GeneratorData {
  ...
  configGeneratorOptions: BuildscaleJsonConfiguration['release']['groups'][number]['version']['generatorOptions'];
  ...
  try {
    ...
    if (err.message.startsWith('Unable to resolve')) {
      ...
      try {
         ...
         * the @buildscale/js generator config in their buildscale.json so we need to be clear about what the problem is)
         */
        if (collectionName === '@buildscale/js') {
          throw new Error(
            'The @buildscale/js plugin is required in order to version your JavaScript packages. Please install it and try again.'
          );
        }
        throw new Error(
