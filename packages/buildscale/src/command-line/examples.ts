...
export const examples: Record<string, Example[]> = {
  ...
  affected: [
    ...
    },
    {
      ...
      description:
        'Run lint, test, and build targets for affected projects. Requires Buildscale v15.4+',
    },
    {
    ...
    {
      command: 'affected -t build --tag=$BUILDSCALE_TASK_TARGET_PROJECT:latest',
  ...
  list: [
    ...
    {
      command: 'list @buildscale/web',
      description:
        'List the generators and executors available in the `@buildscale/web` plugin if it is installed (If the plugin is not installed `buildscale` will show advice on how to add it to your workspace)',
    },
  ],
  'run-many': [
    {
    ...
    },
    {
      ...
      description:
        'Run lint, test, and build targets for all projects. Requires Buildscale v15.4+',
    },
  ],
  migrate: [
    {
      ...
      description:
        'Update all Buildscale plugins to "latest". This will generate migrations.json',
    },
    {
      command: 'migrate 9.0.0',
      description:
        'Update all Buildscale plugins to "9.0.0". This will generate migrations.json',
    },
    {
      command:
        'migrate @buildscale/workspace@9.0.0 --from="@buildscale/workspace@8.0.0,@buildscale/node@8.0.0"',
      description:
        'Update @buildscale/workspace and generate the list of migrations starting with version 8.0.0 of @buildscale/workspace and @buildscale/node, regardless of what is installed locally',
    },
    {
      command:
        'migrate @buildscale/workspace@9.0.0 --to="@buildscale/react@9.0.1,@buildscale/angular@9.0.1"',
      description:
        'Update @buildscale/workspace to "9.0.0". If it tries to update @buildscale/react or @buildscale/angular, use version "9.0.1"',
    },
    {
    ...
    {
      command: 'migrate latest --from=buildscale@14.5.0 --exclude-applied-migrations',
      description:
        'Collect package updates and migrations starting with version 14.5.0 of "buildscale" (and Buildscale first-party plugins), regardless of what is installed locally, while excluding migrations that should have been applied on previous updates',
    },
    {
  ...
  watch: [
    {
      command:
        'watch --projects=app -- echo \\$BUILDSCALE_PROJECT_NAME \\$BUILDSCALE_FILE_CHANGES',
      description:
        'Watch the "app" project and echo the project name and the files that changed',
    },
    {
      command:
        'watch --projects=app1,app2 --includeDependentProjects -- echo \\$BUILDSCALE_PROJECT_NAME',
      description:
        'Watch "app1" and "app2" and echo the project name whenever a specified project or its dependencies change',
    },
    {
      command: 'watch --all -- echo \\$BUILDSCALE_PROJECT_NAME',
      description:
  ...
  add: [
    {
      command: 'add @buildscale/react',
      description:
        'Install the `@buildscale/react` package matching the installed version of the `buildscale` package and run its `@buildscale/react:init` generator',
    },
    {
      command: 'add non-core-buildscale-plugin',
      description:
    ...
    {
      command: 'add @buildscale/react@17.0.0',
      description:
