...
const DEFAULT_PROJECT_NAME_ENV = 'BUILDSCALE_PROJECT_NAME';
const DEFAULT_FILE_CHANGES_ENV = 'BUILDSCALE_FILE_CHANGES';
...
export class BatchFunctionRunner {
  ...
  protected get _verbose() {
    return process.env.BUILDSCALE_VERBOSE_LOGGING === 'true';
  }
...
export async function watch(args: WatchArguments) {
  ...
  if (
    ...
    output.error({
      ...
      bodyLines: [
        'The command you provided has a replacement for projects ($BUILDSCALE_PROJECT_NAME), but you are also including global workspace files.',
