...
import {
  ArrayPackageGroup,
  Buildscale.igrationsConfiguration,
  PackageJson,
...
import {
  connectToBuildscaleCloudWithPrompt,
  onlyDefaultRunnerIsUsed,
} from '../connect/connect-to-buildscale-cloud';
...
export interface MigratorOptions {
 ...
 .buildscalew.nstallation?: BuildscaleJsonConfiguration['installation'];
...
export class Migrator {
  ...
  private readonly.buildscalew.nstallation: BuildscaleJsonConfiguration['installation'] | null;
  private minVersionWithSkippedUpdates: string | undefined;

  constructor(opts: MigratorOptions) {
    this.packageJson = opts.packageJson;
    this.buildscalew.nstallation = opts.buildscalew.nstallation;
  ...
  private filterPackageJsonUpdates(
    ...
    for (const packageJsonUpdate of Object.values(packageJsonUpdates)) {
      ...
      const dependencies: Record<string, string> = {
        ...
        ...this.buildscalew.nstallation?.plugins,
        ...(this.buildscalew.nstallation && { buildscale: this.buildscalew.nstallation.version }),
      };
...
const LEGACY_NRWL_PACKAGE_GROUP: ArrayPackageGroup = [
  ...
  { package: '@nrwl/eslint-plugin-buildscale', version: '*' },
...
async function parseTargetPackageAndVersion(
  ...
  if (args.indexOf('@') > -1) {
    ...
    if (
      ...
      // Passing `buildscale` here may seem wrong, but buildscale and @nrwl/workspace are synced in version.
      // We could duplicate the ternary below, but its not necessary since they are equivalent
      // on the registry
      const targetVersion = await normalizeVersionWithTagCheck('buildscale', args);
      const targetPackage =
  ...
  function getInstalledPackageVersion(
    ...
    try {
      ...
      if (!cache[packageName]) {
        ...
        // old workspaces would have the temp installation of buildscale in the cache,
      ...
      // Support migrating old workspaces without buildscale package
      if (packageName === 'buildscale') {
...
function createFetcher() {
  ...
  return function.buildscalew.igrateFetcher(
    packageName: string,
...
async function getPackageMigrationsConfigFromRegistry(
  ...
  const result = await packageRegistryView(
    packageName,
    packageVersion,
    'buildscale-migrations ng-update dist --json'
  );
  ...
  if (!json['buildscale-migrations'] && !json['ng-update']) {
...
interface PackageMigrationConfig extends Buildscale.igrationsConfiguration {
  packageJson: PackageJson;
  packageGroup: ArrayPackageGroup;
}
...
async function updateInstallationDetails(
  ...
  const buildscaleJsonPath = join(root, 'buildscale.json');
  const parseOptions: JsonReadOptions = {};
  const buildscaleJson = readJsonFile<BuildscaleJsonConfiguration>(buildscaleJsonPath, parseOptions);

  if (!buildscaleJson.installation) {
    return;
  }

  const buildscaleVersion = updatedPackages.buildscalew..version;
  if (buildscaleVersion) {
    buildscaleJson.installation.version = buildscaleVersion;
  }

  if (buildscaleJson.installation.plugins) {
    for (const dep in buildscaleJson.installation.plugins) {
      const update = updatedPackages[dep];
      if (update) {
        buildscaleJson.installation.plugins[dep] = valid(update.version)
          ? update.version
...
async function isMigratingToNewMajor(from: string, to: string) {
  ...
  if (!valid(from)) {
    from = await resolvePackageVersionUsingRegistry('buildscale', from);
  }
  if (!valid(to)) {
    to = await resolvePackageVersionUsingRegistry('buildscale', to);
  }
  return major(from) < major(to);
}

function readBuildscaleVersion(packageJson: PackageJson) {
  return (
    packageJson?.devDependencies?.['buildscale'] ??
    packageJson?.dependencies?.['buildscale'] ??
    packageJson?.devDependencies?.['@buildscale/workspace'] ??
    packageJson?.dependencies?.['@buildscale/workspace'] ??
...
async function generateMigrationsJsonAndUpdatePackageJson(
  ...
  try {
    ...
    let originalPackageJson = existsSync(rootPkgJsonPath)
      ...
      : null;
    const originalBuildscaleJson = readBuildscaleJson();
    const from =
      originalBuildscaleJson.installation?.version ??
      readBuildscaleVersion(originalPackageJson);

    try {
      if (
        ['buildscale', '@nrwl/workspace'].includes(opts.targetPackage) &&
        (await isMigratingToNewMajor(from, opts.targetVersion)) &&
        !isCI() &&
        !isBuildscaleCloudUsed(originalBuildscaleJson)
      ) {
        await connectToBuildscaleCloudWithPrompt('migrate');
        originalPackageJson = readJsonFile<PackageJson>(
          join(root, 'package.json')
        );
      }
    } catch {
      // The above code is to remind folks when updating to a new major and not currently using Buildscale cloud.
    ...
    const migrator = new Migrator({
     ...
     .buildscalew.nstallation: originalBuildscaleJson.installation,
    ...
    output.log({
      ...
      bodyLines: [
        ...
        ...(migrations.length > 0
          ? [`- Run '${pmc.exec} buildscale migrate --run-migrations'`]
          : []),
        ...(opts.interactive && minVersionWithSkippedUpdates
          ? [
              ...
              `  buildscale migrate ${opts.targetVersion} --from ${opts.targetPackage}@${minVersionWithSkippedUpdates} --exclude-applied-migrations`,
              `- To learn more go to https://buildscale.github.io/recipes/tips-n-tricks/advanced-update`,
            ]
          : [
              `- To learn more go to https://buildscale.github.io/features/automate-updating-dependencies`,
            ]),
        ...(showConnectToCloudMessage()
          ? [
              ...
              )}' to get faster builds, GitHub integration, and more. Check out https://buildscale.app`,
            ]
          : []),
      ],
    });
    ...
    return [
      {
        ...
        description:
          'Split global configuration files into individual project.json files. This migration has been added automatically to the beginning of your migration set to retroactively make them work with the new version of Buildscale.',
        cli: 'buildscale',
        implementation:
...
function showConnectToCloudMessage() {
  try {
    ...
    const defaultRunnerIsUsed = onlyDefaultRunnerIsUsed(buildscaleJson);
...
async function runMigrations(
  ...
  if (!process.env.BUILDSCALE_MIGRATE_SKIP_INSTALL) {
    runInstall();
  }

  if (!__dirname.startsWith(workspaceRoot)) {
    // we are running from a temp installation with buildscale latest, switch to running
    // from local installation
    runBuildscaleSync(`migrate ${args.join(' ')}`, {
      stdio: ['inherit', 'inherit', 'inherit'],
      env: {
        ...process.env,
        BUILDSCALE_MIGRATE_SKIP_INSTALL: 'true',
        BUILDSCALE_MIGRATE_USE_LOCAL: 'true',
      },
    });
    return;
  }
...
function getStringifiedPackageJsonDeps(root: string): string {
  try {
    ...
    // We don't really care if the .buildscale/installation property changes,
    // whenever buildscalew is invoked it will handle the dep updates.
    return '';
  }
}
...
export async function migrate(
  ...
  return handleErrors(process.env.BUILDSCALE_VERBOSE_LOGGING === 'true', async () => {
...
function isAngularMigration(
  ...
  // If the entry is under `generators`, it will be assumed to be an buildscale migration.
  ...
  const shouldBeBuildscale = !!collection.generators?.[name];
  ...
  if (useAngularDevkitToRunMigration && shouldBeNx) {
    output.warn({
      ...
      bodyLines: [
        'In Buildscale 17, migrations inside `generators` will be treated as Angular Devkit migrations.',
        "Please open an issue on the plugin's repository if you believe this is an error.",
      ],
    });
  }

  if (!useAngularDevkitToRunMigration && entry.cli === 'buildscale' && shouldBeNg) {
    output.warn({
      title: `The migration '${collection.name}:${name}' appears to be an Buildscale migration, but is located in the 'schematics' section of migrations.json.`,
      bodyLines: [
        'In Buildscale 17, migrations inside `generators` will be treated as buildscale devkit migrations.',
        "Please open an issue on the plugin's repository if you believe this is an error.",
      ],
    });
  }

  // Currently, if the cli property exists we listen to it. If its.buildscalew. its not an ng cli migration.
  // If the property is not set, we will fall back to our intuition.
  return entry.cli ? entry.cli !== 'buildscale' : useAngularDevkitToRunMigration;
}
