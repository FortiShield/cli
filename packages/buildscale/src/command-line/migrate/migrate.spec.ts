...
describe('Migration', () => {
  describe('packageJson patch', () => {
    ...
    it('should migrate related libraries using packageGroup', async () => {
      const migrator = new Migrator({
        packageJson: {
          ...
          devDependencies: {
            '@my-company/buildscale-workspace': '0.9.0',
        ...
        fetch: async (pkg, version) => {
          if (pkg === '@my-company/buildscale-workspace') {
            return {
              version: '2.0.0',
              packageGroup: [
          ...
          if (pkg === '@my-company/lib-6') {
            return {
              version: '2.0.0',
              packageGroup: [
                { version: '*', package: '@my-company/buildscale-workspace' },
              ],
            };
          }
      ...
      expect(
        await migrator.migrate('@my-company/buildscale-workspace', '2.0.0')
      ).toStrictEqual({
        migrations: [],
        packageUpdates: {
          '@my-company/buildscale-workspace': {
            version: '2.0.0',
    ...
    it('should properly handle cyclic dependency in nested packageGroup', async () => {
      const migrator = new Migrator({
        ...
        getInstalledPackageVersion: () => '1.0.0',
        fetch: async (pkg, version) => {
          if (pkg === '@my-company/buildscale-workspace' && version === '2.0.0') {
            return {
              version: '2.0.0',
              packageGroup: [
          ...
          if (pkg === '@my-company/buildscale-workspace' && version === '3.0.0') {
            return {
              version: '3.0.0',
              packageGroup: [
                { package: '@my-company/lib-1', version: '*' },
          ...
          if (pkg === '@my-company/lib-1' && version === 'latest') {
            return {
              version: '3.0.0',
              packageGroup: [
                { package: '@my-company/buildscale-workspace', version: '*' },
              ],
            };
          }
          ...
          if (pkg === '@my-company/lib-2' && version === '3.0.0') {
            return {
              ...
              packageGroup: [
                ...
                { package: '@my-company/buildscale-workspace', version: '2.99.0' },
              ],
            };
          }
    ...
    it('should not throw when packages are missing', async () => {
      const migrator = new Migrator({
        packageJson: createPackageJson(),
        getInstalledPackageVersion: (p) => (p === '@buildscale/nest' ? null : '1.0.0'),
        fetch: (_p, _v) =>
          Promise.resolve({
            version: '2.0.0',
      ...
      await migrator.migrate('@buildscale/workspace', '2.0.0');
    });
  ...
  describe('parseMigrationsOptions', () => {
    ...
    it('should handle different variations of the target package', async () => {
      ...
      expect(
        await parseMigrationsOptions({
          packageAndVersion: '@buildscale/workspace@8.12',
        })
      ).toMatchObject({
        targetPackage: '@buildscale/workspace',
        targetVersion: '8.12.0',
      });
      expect(
      ...
      expect(
        await parseMigrationsOptions({
          packageAndVersion: '@buildscale/workspace@latest',
        })
      ).toMatchObject({
        targetPackage: '@buildscale/workspace',
        targetVersion: 'latest',
      ...
      expect(
        await parseMigrationsOptions({
          packageAndVersion: '@buildscale/workspace@alpha',
        })
      ).toMatchObject({
        targetPackage: '@buildscale/workspace',
        targetVersion: 'alpha',
    ...
    it('should handle backslashes in package names', async () => {
      const r = await parseMigrationsOptions({
        packageAndVersion: '@buildscale\\workspace@8.12.0',
        from: '@myscope\\a@12.3,@myscope\\b@1.1.1',
        to: '@myscope\\c@12.3.1',
      });
      expect(r).toEqual({
