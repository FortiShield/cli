...
export const yargsMigrateCommand: CommandModule = {
  ...
  - Migrate packages and create migrations.json (e.g., buildscale migrate @buildscale/workspace@latest)
  - Run migrations (e.g., buildscale migrate --run-migrations=migrations.json). Use flag --if-exists to run migrations only if the migrations file exists.`,
  builder: (yargs) =>
    linkToBuildscaleDevAndExamples(withMigrationOptions(yargs), 'migrate'),
  handler: () => {
    runMigration();
    process.exit(0);
  },
...
function withMigrationOptions(yargs: Argv) {
  const defaultCommitPrefix = 'chore: [buildscale migration] ';

  return yargs
    .positional('packageAndVersion', {
      describe: `The target package and version (e.g, @buildscale/workspace@16.0.0)`,
      type: 'string',
    })
    ...
    .option('from', {
      describe:
        'Use the provided versions for packages instead of the ones installed in node_modules (e.g., --from="@buildscale/react@16.0.0,@buildscale/js@16.0.0")',
      type: 'string',
    })
    .option('to', {
      describe:
        'Use the provided versions for packages instead of the ones calculated by the migrator (e.g., --to="@buildscale/react@16.0.0,@buildscale/js@16.0.0")',
      type: 'string',
    })
...
function runMigration() {
  const runLocalMigrate = () => {
    runBuildscaleSync(`_migrate ${process.argv.slice(3).join(' ')}`, {
      stdio: ['inherit', 'inherit', 'inherit'],
    });
  };
  if (process.env.BUILDSCALE_MIGRATE_USE_LOCAL === undefined) {
    const p = buildscaleCliPath();
    if (p === null) {
      runLocalMigrate();
    } else {
...
function buildscaleCliPath() {
  const version = process.env.BUILDSCALE_MIGRATE_CLI_VERSION || 'latest';
  try {
    ...
    return path.join(tmpDir, `node_modules`, '.bin', 'buildscale');
  } catch (e) {
    console.error(
