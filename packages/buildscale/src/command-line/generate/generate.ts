...
function throwInvalidInvocation(availableGenerators: string[]) {
  throw new Error(
    `Specify the generator name (e.g., buildscale generate ${availableGenerators.join(
      ', '
    )})`
  );
}
...
export async function generate(cwd: string, args: { [k: string]: any }) {
  ...
  return handleErrors(verbose, async () => {
    const buildscaleJsonConfiguration = readBuildscaleJson();
    ...
    if (deprecated) {
      logger.warn(
        [
          `${BUILDSCALE_PREFIX}: ${opts.collectionName}:${normalizedGeneratorName} is deprecated`,
          `${deprecated}`,
        ].join('\n')
      );
    }
    if (!opts.quiet && !opts.help) {
      logger.info(
        `BUILDSCALE Generating ${opts.collectionName}:${normalizedGeneratorName}`
      );
    }
