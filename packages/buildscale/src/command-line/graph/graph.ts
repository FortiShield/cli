...
import { readBuildscaleJson, workspaceLayout } from '../../config/configuration';
import {
  ProjectFileMap,
  ProjectGraph,
...
import { readFileMapCache } from '../../project-graph/buildscale-deps-cache';
    ...
    if (ext === '.html') {
      ...
      output.warn({
        ...
        bodyLines: [
          ...
          '- criticalPath',
          '',
          'These fields will be removed in Buildscale 19. If you need to see which projects were affected, use `buildscale show projects --affected`.',
        ],
      });
...
async function createTaskGraphClientResponse(
  ...
  const taskGraphs = getAllTaskGraphsForWorkspace(buildscaleJson, graph);
...
interface GraphJsonResponse {
  ...
  /**
   * @deprecated To see affected projects, use `buildscale show projects --affected`. This will be removed in Buildscale 19.
   */
  affectedProjects?: string[];
...
async function createJsonOutput(
  ...
  if (targets?.length) {
    ...
    const hasher = createTaskHasher(rawGraph, readBuildscaleJson());
