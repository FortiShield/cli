...
export async function addBuildscaleToCraRepo(options: Options) {
  if (!options.force) {
...
async function reorgnizeWorkspaceStructure(options: NormalizedOptions) {
  ...
  const buildCommand = options.integrated
    ? `npx buildscale build ${options.reactAppName}`
    : 'npm run build';
  printFinalMessage({
    learnMoreLink:
  ...
  output.note({
    title: 'First time using Buildscale. Check out this interactive Buildscale tutorial.',
    bodyLines: [
      `https://buildscale.github.io/react-tutorial/1-code-generation`,
      ` `,
      `Prefer watching videos? Check out this free Buildscale course on Egghead.io.`,
      `https://egghead.io/playlists/scale-react-development-with-buildscale-4038`,
    ],
  });
...
function createTempWorkspace(options: NormalizedOptions) {
  ...
  execSync(
    ...
    } create-buildscale-workspace@latest temp-workspace --appName=${
      options.reactAppName
    } --preset=react-monorepo --style=css --bundler=${
      options.isVite ? 'vite' : 'webpack'
    } --packageManager=${options.packageManager} ${
      options.buildscaleCloud ? '--buildscaleCloud=yes' : '--buildscaleCloud=skip'
    } ${options.addE2e ? '--e2eTestRunner=cypress' : '--e2eTestRunner=none'}`,
    { stdio: [0, 1, 2] }
  );

  output.log({ title: '👋 Welcome to Buildscale.' });
...
function moveFilesToTempWorkspace(options: NormalizedOptions) {
  output.log({ title: '🚚 Moving your React app in your new Buildscale workspace' });
...
async function addBundler(options: NormalizedOptions) {
  if (options.isVite) {
    ...
    output.log({
      title: '🛬 Skip CRA preflight check since Buildscale manages the monorepo',
    });
