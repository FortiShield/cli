...
import { addVsCodeRecommendedExtensions, createBuildscaleJsonFile } from '../utils';
import type {
  AngularJsonConfig,
...
export async function setupStandaloneWorkspace(
  ...
  replaceNgWithBuildscaleInPackageJsonScripts(repoRoot);
    ...
    writeJsonFile(join(project.root, 'project.json'), {
      $schema: normalizePath(
        relative(
          ...
          join(repoRoot, 'node_modules/buildscale/schemas/project-schema.json')
        )
      ),
      name: projectName,
      ...project,
...
function createBuildscaleJson(
  ...
  createBuildscaleJsonFile(repoRoot, [], cacheableOperations, {});

  const buildscaleJson = readJsonFile<BuildscaleJsonConfiguration>(join(repoRoot, 'buildscale.json'));
  ...
  if (workspaceTargets.includes('server')) {
    buildscaleJson.targetDefaults.server = {
      ...buildscaleJson.targetDefaults.server,
      inputs: ['production', '^production'],
    };
  }
  if (workspaceTargets.includes('test')) {
    ...
    buildscaleJson.targetDefaults.test = {
      ...buildscaleJson.targetDefaults.test,
      inputs,
    };
  }
  if (workspaceTargets.includes('lint')) {
    ...
    buildscaleJson.targetDefaults.lint = {
      ...buildscaleJson.targetDefaults.lint,
      inputs,
    };
  }
  if (workspaceTargets.includes('e2e')) {
    buildscaleJson.targetDefaults.e2e = {
      ...buildscaleJson.targetDefaults.e2e,
      inputs: ['default', '^production'],
    };
  }
  // Angular 15 workspaces still support defaultProject. Support was removed in Angular 16.
  buildscaleJson.defaultProject = angularJson.defaultProject;
  writeJsonFile(join(repoRoot, 'buildscale.json'), buildscaleJson);
}
...
function replaceNgWithBuildscaleInPackageJsonScripts(repoRoot: string): void {
  const packageJsonPath = join(repoRoot, 'package.json');
  const packageJson = readJsonFile<PackageJson>(packageJsonPath);
  packageJson.scripts ??= {};
  Object.keys(packageJson.scripts).forEach((script) => {
    packageJson.scripts[script] = packageJson.scripts[script]
      .replace(/.buildscalew./, 'buildscale')
      .replace(/^ng /, .buildscalew.')
      .replace(/ ng /g, ' buildscale ');
  });
