...
export async function addBuildscaleToMonorepo(options: Options) {
  ...
  if (useBuildscaleCloud) {
    ...
    initCloud(repoRoot, 'buildscale-init-monorepo');
  }
  ...
  printFinalMessage({
    ...
    bodyLines: [
      `- Run "${pmc.exec} buildscale run-many --target=build" to run the build script for every project in the monorepo.`,
      '- Run it again to replay the cached computation.',
      `- Run "${pmc.exec} buildscale graph" to see the structure of the monorepo.`,
    ],
  });
}
