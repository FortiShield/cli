...
export async function format(
  ...
  const patterns = (await getPatterns({ ...args, ...buildscaleArgs } as any)).map(
  ...
  switch (command) {
    case 'write':
      sortTsConfig();
      addRootConfigFiles(chunkList, buildscaleArgs);
...
async function getPatterns(
  args: BuildscaleArgs & { libsAndApps: boolean; _: string[] }
): Promise<string[]> {
  ...
  try {
    ...
    // exclude patterns in .buildscaleignore or .gitignore
...
function addRootConfigFiles(chunkList: string[][], buildscaleArgs: BuildscaleArgs): void {
  if (buildscaleArgs.all) {
    return;
  }
  const chunk = [];
  ...
  // }
  ['buildscale.json', getRootTsConfigFileName()]
    .filter(Boolean)
