...
export function getExecutorInformation(
  ...
  try {
    ...
    const res = {
      ...
      hasherFactory,
      isNgCompat,
      isBuildscaleExecutor: !isNgCompat,
    };
...
function readExecutorJson(
  ...
  const { json: packageJson, path: packageJsonPath } = readPluginPackageJson(
    ...
    projects,
    root
      ? [root, __dirname, process.cwd(), ...getBuildscaleRequirePaths()]
      : [__dirname, process.cwd(), ...getBuildscaleRequirePaths()]
  );
  const executorsFile = packageJson.executors ?? packageJson.builders;

  if (!executorsFile) {
    throw new Error(
      `The "${nodeModule}" package does not support Buildscale executors.`
    );
  }
