...
import { yargsBuildscaleInfixCommand, yargsRunCommand } from './command-object';
...
describe('run-one command setup', () => {
  ...
  describe('infix notation', () => {
    ...
    it('should parse with missing project', () => {
      const parsed = getParsedArgs(['serve', '--prod'], yargsBuildscaleInfixCommand);
...
function getParsedInfixArgs(args: string[]) {
  return getParsedArgs(args, yargsBuildscaleInfixCommand, 0);
}
