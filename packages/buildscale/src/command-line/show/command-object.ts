...
export interface BuildscaleShowArgs {
  json?: boolean;
}

export type ShowProjectsOptions = BuildscaleShowArgs & {
  exclude: string;
  files: string;
  uncommitted: any;
  untracked: any;
...
export type ShowProjectOptions = BuildscaleShowArgs & {
  projectName: string;
  web?: boolean;
  verbose: boolean;
};
...
const showProjectsCommand: CommandModule<BuildscaleShowArgs, ShowProjectsOptions> = {
  command: 'projects',
  ...
  handler: (args) => {
    return handleErrors(
      args.verbose ?? process.env.BUILDSCALE_VERBOSE_LOGGING === 'true',
      async () => {
...
const showProjectCommand: CommandModule<BuildscaleShowArgs, ShowProjectOptions> = {
