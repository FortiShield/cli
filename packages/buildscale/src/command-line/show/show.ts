...
export async function showProjectsHandler(
  ...
  if (args.affected) {
    const touchedFiles = await getTouchedFiles(buildscaleArgs);
    graph = await getAffectedGraph(touchedFiles, buildscaleJson, graph);
  }
  ...
  if (args.exclude) {
    const excludedProjects = findMatchingProjects(buildscaleArgs.exclude, graph.nodes);
...
function getAffectedGraph(
  ...
  return filterAffected(graph, touchedFiles, buildscaleJson);
}

async function getTouchedFiles(buildscaleArgs: BuildscaleArgs): Promise<FileChange[]> {
