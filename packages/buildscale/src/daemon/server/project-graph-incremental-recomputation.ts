...
import {
  FileMapCache,
  buildscaleProjectGraph,
  readFileMapCache,
  writeCache,
} from '../../project-graph/buildscale-deps-cache';
import {
...
export async function getCachedSerializedProjectGraphPromise(): Promise<SerializedProjectGraph> {
  try {
    ...
    // reset the wait time
    waitPeriod = 100;
    await resetInternalStateIfBuildscaleDepsMissing();
...
async function processCollectedUpdatedAndDeletedFiles(
  ...
  try {
    ...
    // if Buildscale requests the project graph prior to the error being fixed,
...
async function processFilesAndCreateAndSerializeProjectGraph(
  ...
  try {
    ...
    const buildscaleJson = readBuildscaleJson(workspaceRoot);
...
async function resetInternalStateIfBuildscaleDepsMissing() {
  try {
    if (!fileExists(buildscaleProjectGraph) && cachedSerializedProjectGraphPromise) {
