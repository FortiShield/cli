...
function daemonIsOutdated(): boolean {
  return buildscaleVersionChanged() || lockFileHashChanged();
}

function buildscaleVersionChanged(): boolean {
  return buildscaleVersion !== getInstalledBuildscaleVersion();
}

const buildscalePackageJsonPath = require.resolve('buildscale/package.json');

function getInstalledBuildscaleVersion() {
  try {
    const { version } = readJsonFile<PackageJson>(buildscalePackageJsonPath);
    return version;
  } catch (e) {
