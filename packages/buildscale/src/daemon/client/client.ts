...
import { hasBuildscaleJson } from '../../config/buildscale-json';
...
const DAEMON_ENV_SETTINGS = {
  ...process.env,
  BUILDSCALE_PROJECT_GLOB_CACHE: 'false',
  BUILDSCALE_CACHE_PROJECTS_CONFIG: 'false',
};
...
export class DaemonClient {
  constructor(private readonly buildscaleJson: BuildscaleJsonConfiguration) {
    this.reset();
  }
  ...
  enabled() {
    if (this._enabled === undefined) {
      ...
      const useDaemonProcessOption =
        this.buildscaleJson.useDaemonProcess ??
        this.buildscaleJson.tasksRunnerOptions?.['default']?.options?.useDaemonProcess;
      const env = process.env.BUILDSCALE_DAEMON;
  ...
  private handleMessage(serializedResult: string) {
    try {
      ...
      this.currentReject(
        daemonProcessException(
          [
            'Could not deserialize response from Buildscale daemon.',
            `Message: ${e.message}`,
            '\n',
            `Received:`,
            endOfResponse,
    ...
    return new Promise((resolve, reject) => {
      const id = setInterval(async () => {
        if (await this.isServerAvailable()) {
          ...
          reject(
            daemonProcessException(
              'Failed to start or connect to the Buildscale Daemon process.'
            )
          );
        } else {
          attempts++;
...
export const daemonClient = new DaemonClient(readBuildscaleJson());
...
function buildscaleJsonIsNotPresent() {
  return !hasBuildscaleJson(workspaceRoot);
}
