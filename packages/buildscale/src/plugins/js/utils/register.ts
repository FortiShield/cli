...
import { logger, BUILDSCALE_PREFIX, stripIndent } from '../../../utils/logger';
 ...
 *
 * @returns cleanup function
 * @deprecated This signature will be removed in Buildscale v19. You should pass the full path to the tsconfig in the first argument.
 */
...
export function getTranspiler(compilerOptions: CompilerOptions) {
  const preferTsNode = process.env.BUILDSCALE_PREFER_TS_NODE === 'true';
...
function warnTsNodeUsage() {
  logger.warn(
    stripIndent(`${BUILDSCALE_PREFIX} Falling back to ts-node for local typescript execution. This may be a little slower.
...
function warnNoTsconfigPaths() {
  logger.warn(
    stripIndent(`${BUILDSCALE_PREFIX} Unable to load tsconfig-paths, workspace libraries may be inaccessible.
...
function warnNoTranspiler() {
  logger.warn(
    stripIndent(`${BUILDSCALE_PREFIX} Unable to locate swc-node or ts-node. Buildscale will be unable to run local ts files without transpiling.
  - To fix this, ensure @swc-node/register and @swc/core have been installed`)
  );
}
