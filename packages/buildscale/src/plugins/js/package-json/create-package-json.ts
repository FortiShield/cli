  ...
  if (existsSync(projectPackageJsonPath)) {
    try {
      ...
      if (graph.nodes[projectName].data.root === '.') {
        // TODO: We should probably think more on this - Buildscale can't always
        // detect all external dependencies, and there's not a way currently
        // to tell Buildscale that we need one of these deps. For non-standalone projects
        ...
        // If Buildscale doesn't pick up a dep, say some css lib that is only imported in a .scss file,
...
export function findProjectsNpmDependencies(
  ...
  const { selfInputs, dependencyInputs } = target
    ? getTargetInputs(readBuildscaleJson(), projectNode, target)
