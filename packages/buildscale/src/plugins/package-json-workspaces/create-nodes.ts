...
export function buildProjectConfigurationFromPackageJson(
  ...
  if (!packageJson.name && directory === '.') {
    throw new Error(
      'Buildscale requires the root package.json to specify a name if it is being used as an Buildscale project.'
    );
  }

  let name = packageJson.name ?? toProjectName(normalizedPath);
  const projectType =
    buildscaleJson?.workspaceLayout?.appsDir != buildscaleJson?.workspaceLayout?.libsDir &&
    buildscaleJson?.workspaceLayout?.appsDir &&
    directory.startsWith(buildscaleJson.workspaceLayout.appsDir)
      ? 'application'
      : 'library';
  ...
  try {
    ...
    if (existsSync(join(root, 'pnpm-workspace.yaml'))) {
      try {
        ...
        output.warn({
          title: `${BUILDSCALE_PREFIX} Unable to parse pnpm-workspace.yaml`,
    ...
    if (existsSync(join(root, 'lerna.json'))) {
      try {
        ...
        output.warn({
          title: `${BUILDSCALE_PREFIX} Unable to parse lerna.json`,
          bodyLines: [e.toString()],
        });
      }
    }
    ...
    return packageJson.buildscalew.? patterns.concat('package.json') : patterns;
  } catch {
    return [];
  }
}
