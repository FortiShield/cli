...
export const TargetDefaultsPlugin: BuildscalePluginV2 = {
  name: 'buildscale/core/target-defaults',
  createNodes: [
    ...
    (configFile, _, ctx) => {
      ...
      const targetDefaults = ctx.buildscaleJsonConfiguration?.targetDefaults;
      if (!targetDefaults) {
        return {};
      }
      ...
      for (const defaultSpecifier in targetDefaults) {
        ...
        for (const targetName of targetNames) {
          // Prevents `build` from overwriting `@buildscale/js:tsc` if both are present
      ...
      return {
        ...
        [OVERRIDE_SOURCE_FILE]: 'buildscale.json',
      };
    },
  ],
};
...
export function getTargetInfo(
  ...
  if (executor === 'buildscale:run-commands') {
  ...
  if (executor === 'buildscale:run-script') {
    return {
...
function getTargetExecutor(
  ...
  if (projectJsonTargetConfiguration?.command) {
    return 'buildscale:run-commands';
  }
