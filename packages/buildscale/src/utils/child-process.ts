...
export function runBuildscaleSync(
  cmd: string,
  options?: ExecSyncOptions & { cwd?: string }
) {
  let baseCmd: string;
  if (existsSync(join(workspaceRoot, 'package.json'))) {
    baseCmd = `${getPackageManagerCommand().exec} buildscale`;
  } else {
    options ??= {};
    ...
    if (process.platform === 'win32') {
      baseCmd = '.\\' + join(`${offsetFromRoot}`, .buildscalew.bat');
    } else {
      baseCmd = './' + join(`${offsetFromRoot}`, 'buildscale');
    }
  }
