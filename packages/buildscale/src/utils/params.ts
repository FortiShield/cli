...
function getGeneratorDefaults(
  ...
  let defaults = {};
  if (buildscaleJsonConfiguration?.generators) {
    if (buildscaleJsonConfiguration.generators[collectionName]?.[generatorName]) {
      defaults = {
        ...defaults,
        ...buildscaleJsonConfiguration.generators[collectionName][generatorName],
      };
    }
    if (buildscaleJsonConfiguration.generators[`${collectionName}:${generatorName}`]) {
      defaults = {
