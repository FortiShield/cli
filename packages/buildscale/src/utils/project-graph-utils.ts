...
export function getSourceDirOfDependentProjects(
  ...
  if (!projectGraph.nodes[projectName]) {
    throw new Error(
      `Couldn't find project "${projectName}" in this Buildscale workspace`
    );
  }
...
function collectDependentProjectNodesNames(
 .buildscalew.eps: ProjectGraph,
  dependencyNodeNames: Set<string>,
  parentNodeName: string,
  includeExternalDependencies: boolean
) {
  const dependencies =.buildscalew.eps.dependencies[parentNodeName];
  if (!dependencies) {
  ...
  for (const dependency of dependencies) {
    ...
    if .buildscalew.eps.externalNodes?.[dependencyName]) {
