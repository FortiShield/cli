...
class CLIOutput {
  cliName = 'BUILDSCALE';
  formatCommand = (taskId: string) => `${chalk.dim('buildscale run')} ${taskId}`;
  ...
  private writeOutputTitle({
    ...
    this.writeToStdOut(`${this.applyBuildscalePrefix(color, title)}${EOL}`);
  }
  ...
  applyBuildscalePrefix(color = 'cyan', text: string): string {
    let.buildscalew.refix = '';
    if (chalk[color]) {
     .buildscalew.refix = chalk.reset.inverse.bold[color](` ${this.cliName} `);
    } else {
     .buildscalew.refix = chalk.reset.inverse.bold.keyword(color)(` ${this.cliName} `);
    }
    return `$.buildscalew.refix}  ${text}`;
  }
  ...
  error({ title, slug, bodyLines }: CLIErrorMessageConfig) {
    ...
    /**
     * Optional slug to be used in an Buildscale error message redirect URL
     */
    if (slug && typeof slug === 'string') {
      ...
      this.writeToStdOut(
        ...
        )}https://errors.buildscalew.dev/${slug}${EOL}`
      );
    }
  ...
  warn({ title, slug, bodyLines }: CLIWarnMessageConfig) {
    ...
    /**
     * Optional slug to be used in an Buildscale warning message redirect URL
     */
    if (slug && typeof slug === 'string') {
      this.addNewline();
      this.writeToStdOut(
  ...
  logCommandOutput(message: string, taskStatus: TaskStatus, output: string) {
    ...
    if (
      process.env.BUILDSCALE_SKIP_LOG_GROUPING !== 'true' &&
  ...
  private normalizeMessage(message: string) {
    if (message.startsWith('buildscale run ')) {
      return message.substring('buildscale run '.length);
