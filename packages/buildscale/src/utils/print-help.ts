...
import { buildscaleVersion } from './versions';
...
// TODO: generalize link generation so it works for non @buildscale plugins as well
function generateLinkOutput({
  ...
  const buildscalePackagePrefix = '@buildscale/';
  const nrwlPackagePrefix = '@nrwl/';
  if (
    !pluginName.startsWith(buildscalePackagePrefix) &&
    !pluginName.startsWith(nrwlPackagePrefix)
  ) {
    return '';
  }

  const link = `https://buildscale.github.io/packages/${pluginName.substring(
    pluginName.startsWith(buildscalePackagePrefix)
      ? buildscalePackagePrefix.length
