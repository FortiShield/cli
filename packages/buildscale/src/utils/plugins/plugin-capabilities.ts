...
import { BuildscalePlugin, readPluginPackageJson } from '../../project-graph/plugins';
import { loadBuildscalePlugin } from '../../project-graph/plugins/loader';
  ...
  try {
    const { json: packageJson, path: packageJsonPath } =
      await readPluginPackageJson(
        pluginName,
        projects,
        getBuildscaleRequirePaths(workspaceRoot)
      );
...
async function tryGetModule(
  ...
  workspaceRoot: string
): Promise<BuildscalePlugin | null> {
  try {
    if (
      packageJson.generators ??
      packageJson.executors ??
      packageJson['buildscale-migrations'] ??
      packageJson['schematics'] ??
      packageJson['builders']
    ) {
      const [pluginPromise] = loadBuildscalePlugin(packageJson.name, workspaceRoot);
  ...
  if (!plugin) {
    ...
    output.note({
      ...
      bodyLines: [
        ...
        `After that, use "${pmc.exec} buildscale g ${pluginName}:init" to add the required peer deps and initialize the plugin.`,
      ],
    });
