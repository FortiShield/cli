...
/**
 * @deprecated Use {@link BuildscaleJsonConfiguration#defaultBase } instead
 */
export interface BuildscaleAffectedConfig {
  /**
...
interface BuildscaleInstallationConfiguration {
  /**
   * Version used for Buildscale
   */
  version: string;
  /**
   * Record<pluginPackageName, pluginVersion>. e.g.
   * plugins: { '@buildscale/angular': '1.0.0' }
   */
  plugins?: Record<string, string>;
}

export interface BuildscaleReleaseVersionConfiguration {
  generator?: string;
...
export interface BuildscaleReleaseChangelogConfiguration {
  /**
   ...
   *
   * By default, the renderer is set to "buildscale/release/changelog-renderer" which buildscale provides out of the box.
   */
  renderer?: string;
  renderOptions?: ChangelogRenderOptions;
}

export interface BuildscaleReleaseGitConfiguration {
  /**
...
export interface BuildscaleReleaseConventionalCommitsConfiguration {
  types?: Record<
    string,
    /**
...
interface BuildscaleReleaseConfiguration {
  /**
  ...
  groups?: Record<
    string, // group name
    {
       ...
       */
      version?: BuildscaleReleaseVersionConfiguration;
      /**
       ...
       * NOTE: git configuration is not supported at the group level, only the root/command level
       */
      changelog?: BuildscaleReleaseChangelogConfiguration | boolean;
      /**
  ...
  changelog?: {
     ...
     */
    git?: BuildscaleReleaseGitConfiguration;
    /**
     ...
     */
    workspaceChangelog?: BuildscaleReleaseChangelogConfiguration | boolean;
    /**
     ...
     * - false = explicitly disable project level changelogs
     */
    projectChangelogs?: BuildscaleReleaseChangelogConfiguration | boolean;
    /**
  ...
  /**
   * If no version config is provided, we will assume that @buildscale/js:release-version
  ...
  version?: BuildscaleReleaseVersionConfiguration & {
    ...
    git?: BuildscaleReleaseGitConfiguration;
    /**
     * A command to run after validation of buildscale release configuration, but before versioning begins.
     * Used for preparing build artifacts. If --dry-run is passed, the command is still executed, but
     * with the BUILDSCALE_DRY_RUN environment variable set to 'true'.
     */
   ...
   */
  git?: BuildscaleReleaseGitConfiguration;
  conventionalCommits?: BuildscaleReleaseConventionalCommitsConfiguration;
}

/**
 * Buildscale.json configuration
 *
 * @note: when adding properties here add them to `allowedWorkspaceExtensions` in adapter/compat.ts
 */
export interface BuildscaleJsonConfiguration<T = '*' | string[]> {
  /**
   * Optional (additional) Buildscale.json configuration file which becomes a base for this one
   */
  extends?: string;
  /**
   * Map of files to projects that implicitly depend on them
   * @deprecated use {@link namedInputs} instead. For more information see https://buildscale.github.io/deprecated/global-implicit-dependencies#global-implicit-dependencies
   */
  ...
  /**
   * Default options for `buildscale affected`
   * @deprecated use {@link defaultBase} instead. For more information see https://buildscale.github.io/deprecated/affected-config#affected-config
   */
  affected?: BuildscaleAffectedConfig;

  /**
   * Default value for --base used by `buildscale affected` and `buildscale format`.
   */
  defaultBase?: string;
  ...
  /**
   * Configuration for Buildscale Plugins
   */
  ...
  /**
   * Configures the Buildscale installation for a repo. Useful for maintaining  a separate
   * set of dependencies for Buildscale + Plugins compared to the base package.json, but also
   * useful for workspaces that don't have a root package.json + node_modules.
   */
  installation?: BuildscaleInstallationConfiguration;

  /**
   * Configuration for `buildscale release` (versioning and publishing of applications and libraries)
   */
  release?: BuildscaleReleaseConfiguration;

  /**
   * If specified Buildscale will use buildscale-cloud by default with the given token.
   * To use a different runner that accepts an access token, define it in {@link tasksRunnerOptions}
   */
  buildscaleCloudAccessToken?: string;

  /**
   * Specifies the url pointing to an instance of buildscale cloud. Used for remote
   * caching and displaying run links.
   */
  buildscaleCloudUrl?: string;

  /**
   * Specifies the encryption key used to encrypt artifacts data before sending it to buildscale cloud.
   */
  buildscaleCloudEncryptionKey?: string;
  ...
  /**
   * Changes the directory used by Buildscale to store its cache.
   */
...
export function readBuildscaleJson(root: string = workspaceRoot): BuildscaleJsonConfiguration {
  const buildscaleJson = join(root, 'buildscale.json');
  if (existsSync(buildscaleJson)) {
    const buildscaleJsonConfiguration = readJsonFile<BuildscaleJsonConfiguration>(buildscaleJson);
    if (buildscaleJsonConfiguration.extends) {
      const extendedBuildscaleJsonPath = require.resolve(buildscaleJsonConfiguration.extends, {
        paths: [dirname(buildscaleJson)],
      });
      const baseBuildscaleJson = readJsonFile<BuildscaleJsonConfiguration>(extendedBuildscaleJsonPath);
      return {
        ...baseBuildscaleJson,
        ...buildscaleJsonConfiguration,
      };
    } else {
      return buildscaleJsonConfiguration;
    }
  } else {
    try {
...
export function hasBuildscaleJson(root: string): boolean {
  const buildscaleJson = join(root, 'buildscale.json');
  return existsSync(buildscaleJson);
}
