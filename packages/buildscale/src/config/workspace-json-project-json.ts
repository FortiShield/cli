import type {
  ...
  BuildscaleReleaseVersionConfiguration,
} from './buildscale-json';

/**
 * @deprecated use ProjectsConfigurations or BuildscaleJsonConfiguration
 */
export interface Workspace extends ProjectsConfigurations, BuildscaleJsonConfiguration {
...
export interface ProjectConfiguration {
  ...
  /**
   * Project specific configuration for `buildscale release`
   */
  release?: {
    version?: Pick<
      BuildscaleReleaseVersionConfiguration,
...
export interface TargetConfiguration<T = any> {
   ...
   *
   * Example: '@buildscale/rollup:rollup'
   */
  executor?: string;

  /**
   * Used as a shorthand for buildscale:run-commands, a command to run.
   */
  command?: string;

  /**
   * List of the target's outputs. The outputs will be cached by the Buildscale computation
   * caching engine.
   */
  outputs?: string[];
  ...
  /**
   * Determines if Buildscale is able to cache a given target.
   */
  cache?: boolean;
