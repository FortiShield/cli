...
import type { CloudTaskRunnerOptions } from './buildscale-cloud-tasks-runner-shell';
...
export class BuildscaleCloudEnterpriseOutdatedError extends Error {
  constructor(url: string) {
    super(`Buildscale Cloud instance hosted at ${url} is outdated`);
  }
}
export class BuildscaleCloudClientUnavailableError extends Error {
  constructor() {
    super('No existing Buildscale Cloud client and failed to download new version');
  }
}

export interface BuildscaleCloudClient {
  configureLightClientRequire: () => (paths: string[]) => void;
  commands: Record<string, () => Promise<void>>;
  buildscaleCloudTasksRunner: TasksRunner<CloudTaskRunnerOptions>;
}
export async function verifyOrUpdateBuildscaleCloudClient(
  options: CloudTaskRunnerOptions
): Promise<{ buildscaleCloudClient: BuildscaleCloudClient; version: string } | null> {
  ...
  if (shouldVerifyInstalledRunnerBundle(currentBundle)) {
    ...
    try {
      ...
      if (e.message === 'Request failed with status code 404' && options.url) {
        throw new BuildscaleCloudEnterpriseOutdatedError(options.url);
      }
      ...
      if (currentBundle === null) {
        throw new BuildscaleCloudClientUnavailableError();
      }

      if (currentBundle.version === 'BUILDSCALE_ENTERPRISE_OUTDATED_IMAGE') {
        throw new BuildscaleCloudEnterpriseOutdatedError(options.url);
      }

      const buildscaleCloudClient = require(currentBundle.fullPath);
      if (buildscaleCloudClient.commands === undefined) {
        throw new BuildscaleCloudEnterpriseOutdatedError(options.url);
      }

      return {
        version: currentBundle.version,
        buildscaleCloudClient,
      };
    }
    ...
    if (verifyBundleResponse.data.valid) {
      ...
      return {
        version: currentBundle.version,
        buildscaleCloudClient: require(currentBundle.fullPath),
      };
    }
    ...
    if (version === 'BUILDSCALE_ENTERPRISE_OUTDATED_IMAGE') {
      throw new BuildscaleCloudEnterpriseOutdatedError(options.url);
    }
    ...
    const buildscaleCloudClient = require(fullPath);

    if (buildscaleCloudClient.commands === undefined) {
      throw new BuildscaleCloudEnterpriseOutdatedError(options.url);
    }
    return { version, buildscaleCloudClient };
  }
...
function shouldVerifyInstalledRunnerBundle(
  ...
  if (process.env.BUILDSCALE_CLOUD_FORCE_REVALIDATE === 'true') {
    return true;
  }
...
async function verifyCurrentBundle(
  ...
  return axios.get('/buildscale-cloud/client/verify', {
...
async function downloadAndExtractClientBundle(
  ...
  try {
    ...
    console.error('Error while updating Buildscale Cloud client bundle');
    throw e;
  }
