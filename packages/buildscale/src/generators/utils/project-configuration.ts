...
import { BuildscalePlugin } from '../../project-graph/plugins';

export { readBuildscaleJson, updateBuildscaleJson } from './buildscale-json';

/**
 * Adds project configuration to the Buildscale workspace.
 *
...
export function addProjectConfiguration(
  ...
  if (!standalone) {
    output.warn({
      title:
        'Buildscale only supports standalone projects. Setting standalone to false is ignored.',
    });
  }
...
export function getRelativeProjectJsonSchemaPath(
  ...
  return normalizePath(
    relative(
      ...
      join(tree.root, 'node_modules/buildscale/schemas/project-schema.json')
    )
  );
}
...
function handleEmptyTargets(
  ...
  if (
    ...
    projectConfiguration[
      '// targets'
    ] = `to see all targets run: buildscale show project ${projectName} --web`;
    projectConfiguration.targets = {};
  } else {
    delete projectConfiguration['// targets'];
  }
