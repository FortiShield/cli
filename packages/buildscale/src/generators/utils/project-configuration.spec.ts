...
describe('project configuration', () => {
  ...
  it('should add a comment to show project details when targets are missing', () => {
    ...
    expect(tree.read('libs/test/project.json', 'utf-8')).toMatchInlineSnapshot(`
      "{
        ...
        "$schema": "../../node_modules/buildscale/schemas/project-schema.json",
        "sourceRoot": "libs/test/src",
        "// targets": "to see all targets run: buildscale show project test --web",
        "targets": {}
      }
      "
    `);
  ...
  describe('JSON schema', () => {
    it('should have JSON $schema in project configuration for standalone projects', () => {
      ...
      expect(projectJson['$schema']).toEqual(
        '../../node_modules/buildscale/schemas/project-schema.json'
      );
    });
  ...
  describe('without buildscale.json', () => {
    beforeEach(() => tree.delete('buildscale.json'));
