...
describe('Run Commands', () => {
  ...
  describe('dotenv', () => {
    ...
    beforeEach(() => {
      ...
      delete process.env.BUILDSCALE_SITE;
    });
    ...
    it('should load the specified .env file instead of the root one', async () => {
      ...
      writeFileSync(devEnv, 'BUILDSCALE_SITE=https://buildscale.github.io/');
      let f = fileSync().name;
      const result = await runCommands(
        {
          commands: [
            {
              command: `echo $BUILDSCALE_SITE >> ${f} && echo $NRWL_SITE >> ${f}`,
            },
          ],
          envFile: devEnv,
          __unparsed__: [],
      ...
      expect(result).toEqual(expect.objectContaining({ success: true }));
      expect(readFile(f)).toEqual('https://buildscale.github.io/');
    });
