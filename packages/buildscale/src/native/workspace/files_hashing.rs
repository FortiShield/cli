...
use crate::native::walker::{buildscale_walker, BuildscaleFile};
use crate::native::workspace::files_archive::{BuildscaleFileHashed, BuildscaleFileHashes};

pub fn full_files_hash(workspace_root: &Path) -> BuildscaleFileHashes {
    let files = buildscale_walker(workspace_root).collect::<Vec<_>>();
    hash_files(files).into_iter().collect()
}

pub fn selective_files_hash(
    workspace_root: &Path,
    mut archived_files: BuildscaleFileHashes,
) -> BuildscaleFileHashes {
    let files = buildscale_walker(workspace_root).collect::<Vec<_>>();
...
fn hash_files(files: Vec<BuildscaleFile>) -> Vec<(String, BuildscaleFileHashed)> {
        ...
        files
            ...
            .filter_map(|file| {
                hash_file_path(&file.full_path)
                    .map(|hash| (file.normalized_path, BuildscaleFileHashed(hash, file.mod_time)))
            })
            .collect::<Vec<_>>()
    } else {
        files
                ...
                chunks.iter().filter_map(|file| {
                    hash_file_path(&file.full_path).map(|hash| {
                        (
                            ...
                            BuildscaleFileHashed(hash, file.mod_time),
                        )
                    })
                })
            })
    ...
    fn should_selectively_hash_workspace() {
        ...
        let archived_files = vec![
            ...
            (
                ...
                BuildscaleFileHashed(String::from("hash5"), 0),
            ),
        ]
        .into_iter()
        .collect::<BuildscaleFileHashes>();
