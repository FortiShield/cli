...
mod tests {
    ...
    fn test_hash_project_config_with_data() {
                ...
                Project {
                    ...
                    targets: HashMap::from([
                        (
                            "build".into(),
                            Target {
                                executor: Some("@buildscale/node:build".into()),
                                options: Some("{}".into()),
                        ...
                        (
                            "test".into(),
                            Target {
                                executor: Some("@buildscale/node:test".into()),
                                options: Some("{}".into()),
                                configurations: Some("{\"production\":{}}".into()),
                                ..Default::default()
                            },
        ...
        let buildscale_project_hash = hash_project_config("buildscale", &projects);
        assert_eq!(buildscale_project_hash.unwrap(), "3244421341483603138");
    ...
    fn test_hash_project_config_with_no_project() {
        ...
        let result = hash_project_config("buildscale", &projects);

        assert!(result.is_err());
        assert_eq!(
            result.unwrap_err().to_string(),
            "Could not find project 'buildscale'"
        );
    }
}
