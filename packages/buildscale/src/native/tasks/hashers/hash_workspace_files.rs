...
pub fn hash_workspace_files(
    ...
    let Some(file_set) = file_set else {
        warn!(
            "{workspace_file_set} does not start with {}. This will throw an error in Buildscale 19.",
            "{workspaceRoot}/"
        );
        return Ok(hash(b""));
    };
...
mod test {
    ...
    fn test_hash_workspace_files() {
        ...
        let buildscaleignore_file = FileData {
            file: ".buildscaleignore".into(),
            hash: "456".into(),
        };
