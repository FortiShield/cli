...
use crate::native::types::{Input, BuildscaleJson};
use crate::native::{
...
pub struct HashPlanner {
    buildscale.json: BuildscaleJson,
...
impl HashPlanner {
    #[napi(constructor)]
    pub fn new(buildscale.json: BuildscaleJson, project_graph: External<ProjectGraph>) -> Self {
        Self {
            buildscale.json,
            project_graph,
        }
    ...
    pub fn get_plans_internal(
        ...
        task_ids
            ...
            .map(|id| {
                ...
                let inputs = get_inputs(task, &self.project_graph, &self.buildscale.json)?;
                ...
                let mut inputs: Vec<HashInstruction> = target
                    ...
                    .into_iter()
                    .chain(vec![
                        HashInstruction::WorkspaceFileSet("{workspaceRoot}/buildscale.json".to_string()),
                        HashInstruction::WorkspaceFileSet("{workspaceRoot}/.gitignore".to_string()),
                        HashInstruction::WorkspaceFileSet("{workspaceRoot}/.buildscaleignore".to_string()),
                    ])
                    .chain(self_inputs)
                    .collect();
    ...
    fn target_input<'a>(
        ...
        // we can only vouch for @buildscale packages's executor dependencies
        // if it's "run commands" or third-party we skip traversing since we have no info what this command depends on
        if target
            .executor
            .as_ref()
            .is_some_and(|e| e.starts_with("@nrwl/") || e.starts_with("@buildscale/"))
        {
    ...
    fn gather_project_inputs(
        ...
        for project in project_inputs {
            ...
            for project in projects {
                let named_inputs =
                    get_named_inputs(&self.buildscale.json, &self.project_graph.nodes[project]);
