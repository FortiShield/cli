...
pub(super) fn get_inputs<'a>(
    ...
    buildscale.json: &'a BuildscaleJson,
    ...
    let named_inputs = get_named_inputs(buildscale.json, project_node);
...
pub(super) fn get_inputs_for_dependency<'a>(
    ...
    let inputs = get_named_inputs(buildscale.json, project);
...
If "{file_set}" is a named input, make sure it is defined in buildscale.json.
"#,
            file_set = s,
...
pub(super) fn get_named_inputs<'a>(
    ...
    let iterable_structs = [&buildscale.json.named_inputs, &project.named_inputs];
