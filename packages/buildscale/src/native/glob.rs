...
pub struct Buildscale.lobSetBuilder {
    included_globs: GlobSetBuilder,
    excluded_globs: GlobSetBuilder,
}

impl Buildscale.lobSetBuilder {
    pub fn new<S: AsRef<str>>(globs: &[S]) -> anyhow::Result<Self> {
        let mut glob_set_builder = Buildscale.lobSetBuilder {
    ...
    pub fn add(&mut self, glob: &str) -> anyhow::Result<&mut Buildscale.lobSetBuilder> {
...
pub struct Buildscale.lobSet {
    included_globs: GlobSet,
    excluded_globs: GlobSet,
}
impl Buildscale.lobSet {
...
pub(crate) fn build_glob_set<S: AsRef<str> + Debug>(globs: &[S]) -> anyhow::Result<NxGlobSet> {
    ...
    Buildscale.lobSetBuilder::new(&result)?.build()
}
...
mod test {
    ...
    fn should_work_with_simple_globs() {
        ...
        assert!(glob_set.is_match("packages/buildscale/package.json"));
    ...
    fn should_not_detect_deeply_nested_package_json() {
        let glob_set = build_glob_set(&["packages/*/package.json"]).unwrap();
        assert!(!glob_set.is_match("packages/buildscale/test-files/package.json"))
    }
    ...
    fn should_detect_node_modules() {
        ...
        assert!(glob_set.is_match("packages/buildscale/node_modules"));
    }
