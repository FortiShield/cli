...
describe('task planner', () => {
  // disable BUILDSCALE_NATIVE_TASK_HASHER for this test because we need to compare the results of the new planner with the old task hasher
  ...
  const tsConfigBaseJson = JSON.stringify({
    compilerOptions: {
      paths: {
        ...
        '@buildscale/grandchild': ['libs/grandchild/src/index.ts'],
      },
    },
  });
  ...
  it('should include npm projects', async () => {
    ...
    const planner = new HashPlanner(buildscaleJson as any, transformed);
  ...
  describe('dependentTasksOutputFiles', () => {
    it('should depend on dependent tasks output files', async () => {
      ...
      const planner = new HashPlanner(buildscaleJson, transformed);
      let plans = await assertHashPlan(
        taskGraph.tasks['parent:build'],
        taskGraph,
        hasher,
