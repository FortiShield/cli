...
pub(super) fn get_buildscale_ignore<P: AsRef<Path>>(origin: P) -> Option<IgnoreFile> {
    let buildscale_ignore_path = PathBuf::from(origin.as_ref()).join(".buildscaleignore");
    if buildscale_ignore_path.exists() {
        Some(IgnoreFile {
            path: buildscale_ignore_path,
