...
use crate::native::watch::utils::{get_ignore_files, get_buildscale_ignore, transform_event};

#[derive(Debug)]
pub struct WatchFilterer {
    pub buildscale_ignore: Option<IgnoreFilter>,
...
impl WatchFilterer {
    fn filter_event(&self, event: &Event, priority: Priority) -> bool {
        ...
        for (path, file_type) in event.paths() {
            ...
            let buildscale_ignore_match_type = if let Some(buildscale_ignore) = &self.buildscale_ignore {
                buildscale_ignore.match_path(path, is_dir)
            } else {
                Match::None
            };

            // if the buildscaleignore file contains this file as a whitelist,
            // we do not want gitignore to filter it out, so it will always pass as true
            if matches!(buildscale_ignore_match_type, Match::Whitelist(_)) {
                trace!(?path, "buildscaleignore whitelist match, ignoring gitignore");
                pass &= true;
            // If the buildscaleignore file contains this file as an ignore,
            // then there's no point in checking the gitignore file
            } else if matches!(buildscale_ignore_match_type, Match::Ignore(_)) {
                trace!(?path, "buildscaleignore ignore match, ignoring gitignore");
                pass &= false;
            } else {
                pass &= self
                    .git_ignore
...
pub(super) async fn create_filter(
    ...
    let buildscale_ignore_file = get_buildscale_ignore(origin);
    ...
    let buildscale_ignore = if let Some(buildscale_ignore_file) = buildscale_ignore_file {
        Some(
            IgnoreFilter::new(origin, &[buildscale_ignore_file])
                .await
                .map_err(anyhow::Error::from)?,
        )
    } else {
