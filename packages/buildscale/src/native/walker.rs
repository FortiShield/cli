...
pub fn buildscale_walker_sync<'a, P>(
    directory: P,
...
/// Walk the directory and ignore files from .gitignore and .buildscaleignore
pub fn buildscale_walker<P>(directory: P) -> impl Iterator<Item = BuildscaleFile>
where
    P: AsRef<Path>,
{
    let directory = directory.as_ref();

    let ignore_glob_set = build_glob_set(&["**/node_modules", "**/.git", "**/.buildscale/cache"])
        .expect("These static ignores always build");

    let mut walker = WalkBuilder::new(directory);
    walker.hidden(false);
    walker.add_custom_ignore_filename(".buildscaleignore");
...
mod test {
    ...
    fn it_walks_a_directory() {
        ...
        let content = buildscale_walker("/does/not/exist").collect::<Vec<_>>();
        assert!(content.is_empty());

        let temp_dir = setup_fs();

        let mut content = buildscale_walker(&temp_dir).collect::<Vec<_>>();
        content.sort();
        let content = content
            .into_iter()
    ...
    fn handles_buildscale_ignore() {
        ...
        // add buildscaleignore file
        temp_dir
            .child(".buildscaleignore")
            .write_str(
                r"baz/
nested/child.txt
nested/child-two/
...
/packages
    ",
        ...
        let mut file_names = buildscale_walker(temp_dir)
            .map(
                |BuildscaleFile {
