...
fn process_file(
    ...
    let mut lines_with_buildscale_ignore_comments: HashSet<usize> = HashSet::new();
    let leading_comments = leading_comments.borrow();
    for (_, comments) in leading_comments.iter() {
        for comment in comments {
            let comment_text = comment.text.trim();

            if comment_text.contains("buildscale-ignore-next-line") {
                let line_where_comment_ends = cm
                    .lookup_line(comment.span.hi)
                    .expect("Comments end on a line");

                lines_with_buildscale_ignore_comments.insert(line_where_comment_ends);
            }
        }
    }
    ...
    let code_is_not_ignored = |(specifier, pos): (String, BytePos)| {
        ...
        if line_with_code > 0 && lines_with_buildscale_ignore_comments.contains(&(line_with_code - 1)) {
            None
        } else {
            Some(specifier)
        }
...
mod find_imports {
    ...
    use crate::native::walker::buildscale_walker;
...
/* prettier-ignore */ /* buildscale-ignore-next-line */
import 'a4'; import 'a5';

/* buildscale-ignore-next-line */ /* prettier-ignore */
import 'a4'; import 'a5';
...
<template>
  ...
  <WelcomeItem>
    ...
    fn should_find_imports_in_tsx_files() {
        ...
        temp_dir
            ...
            .write_str(
                ...
                const BuildscaleProjectGraphViz = dynamic(
    ...
    #[test]
    fn should_ignore_lines_with_buildscale_ignore() {
        let temp_dir = TempDir::new().unwrap();
        temp_dir
            .child("test.ts")
            .write_str(
...
import(myTag`react@${version}`);
    ...
    #[test]
    #[ignore]
    fn should_find_imports_properly_for_all_files_in_buildscale_repo() {
        ...
        let files = buildscale_walker(root.clone())
            .filter(|file| glob.is_match(&file.full_path))
            .map(|file| file.full_path)
            .collect::<Vec<_>>();

        let results: HashMap<_, _> =
            find_imports(HashMap::from([(String::from("buildscale"), files.clone())]))
                .unwrap()
                .into_iter()
        ...
        for dep in deps {
            ...
            if line_with_dep > 0 {
                if let Some(line_before_dep) = cm.get_line(line_with_dep - 1) {
                    ...
                    if trimmed_line == "// buildscale-ignore-next-line"
                        || trimmed_line == "/* buildscale-ignore-next-line */"
                    {
                        continue;
                    }
                }
