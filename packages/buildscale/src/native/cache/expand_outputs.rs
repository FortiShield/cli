...
use crate::native::walker::buildscale_walker_sync;
...
pub fn expand_outputs(directory: String, entries: Vec<String>) -> anyhow::Result<Vec<String>> {
    ...
    let found_paths = buildscale_walker_sync(directory, Some(&negated_globs))
        .filter_map(|path| {
...
pub fn get_files_for_outputs(
    ...
    if !globs.is_empty() {
        // todo(jcammisuli): optimize this as buildscale_walker_sync is very slow on the root directory. We need to change this to only search smaller directories
        let glob_set = build_glob_set(&globs)?;
        let found_paths = buildscale_walker_sync(&directory, None).filter_map(|path| {
            if glob_set.is_match(&path) {
                Some(path.to_normalized_string())
            } else {
                None
    ...
    if !directories.is_empty() {
        for dir in directories {
            ...
            let files_in_dir = buildscale_walker_sync(&dir_path, None).filter_map(|e| {
...
mod test {
    ...
    fn setup_fs() -> TempDir {
        ...
        temp.child("nested")
            .child("deeply")
            .child("buildscale.darwin-arm64.node")
            .touch()
            .unwrap();
    ...
    fn should_expand_outputs() {
        let temp = setup_fs();
        let entries = vec![
            "packages/buildscale/src/native/*.node".to_string(),
        ...
        assert_eq!(
            ...
            vec![
                ...
                "packages/buildscale/src/native/buildscale.darwin-arm64.node",
                "test.txt"
            ]
        );
    }
