import { BuildscaleJsonConfiguration, TargetDefaults } from '../../config/buildscale-json';
...
import { BUILDSCALE_PREFIX } from '../../utils/logger';
 ...
 * @param buildscaleJson The BuildscaleJson configuration
 ...
 * to avoid merging options for @buildscale/js:tsc into something like @buildscale/webpack:webpack.
 *
...
export function isCompatibleTarget(
  ...
  const isRunCommands = a.executor === 'buildscale:run-commands';
  if (isRunCommands) {
  ...
  const isRunScript = a.executor === 'buildscale:run-script';
  if (isRunScript) {
...
export function resolveBuildscaleTokensInOptions<T extends Object | Array<unknown>>(
  object: T,
  project: ProjectConfiguration,
  key: string
): T {
  ...
  for (let [opt, value] of Object.entries(object ?? {})) {
    if (typeof value === 'string') {
      ...
      if (value.includes('{workspaceRoot}')) {
        throw new Error(
          `${BUILDSCALE_PREFIX} The {workspaceRoot} token is only valid at the beginning of an option. (${key})`
        );
      }
      value = value.replace(/\{projectRoot\}/g, project.root);
      result[opt] = value.replace(/\{projectName\}/g, project.name);
    } else if (typeof value === 'object' && value) {
      result[opt] = resolveBuildscaleTokensInOptions(
        value,
        project,
        [key, opt].join('.')
      );
...
export function readTargetDefaultsForTarget(
  ...
  if (executor) {
    ...
    // Use, use buildscale:run-commands if it is present
...
function resolveCommandSyntacticSugar(
  ...
  if (config.executor) {
    throw new Error(
      `${BUILDSCALE_PREFIX} Project at ${key} should not have executor and command both configured.`
    );
  } else {
    return {
      ...config,
