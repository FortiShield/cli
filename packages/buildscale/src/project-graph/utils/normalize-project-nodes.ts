...
import { resolveBuildscaleTokensInOptions } from './project-configuration-utils';
...
export function normalizeProjectTargets(
  ...
  for (const target in targets) {
    ...
    targets[target].options = resolveBuildscaleTokensInOptions(
    ...
    for (const configuration in targets[target].configurations) {
      targets[target].configurations[configuration] = resolveBuildscaleTokensInOptions(
