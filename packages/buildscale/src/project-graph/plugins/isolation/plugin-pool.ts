...
import { LoadedBuildscalePlugin,.buildscalew.luginCache } from '../internal-api';
...
export function loadRemoteBuildscalePlugin(
  plugin: PluginConfiguration,
  root: string
): [Promise<LoadedBuildscalePlugin>, () => void] {
  // this should only really be true when running unit tests within
  // the Buildscale repo. We still need to start the worker in this case,
  // but its typescript.
  ...
  return [
    new Promise<LoadedBuildscalePlugin>((res, rej) => {
      worker.on(
        'message',
...
function createWorkerHandler(
  ...
  onload: (plugin: LoadedBuildscalePlugin) => void,
