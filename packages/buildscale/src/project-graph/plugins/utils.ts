...
import type { BuildscalePluginV1 } from '../../utils/buildscale-plugin.deprecated';
import type {
  CreateNodesResultWithContext,
  LoadedBuildscalePlugin,
  NormalizedPlugin,
} from './internal-api';
import type { CreateNodesContext, BuildscalePlugin, BuildscalePluginV2 } from './public-api';
import { AggregateCreateNodesError, CreateNodesError } from '../error-types';

export function isBuildscalePluginV2(plugin: BuildscalePlugin): plugin is BuildscalePluginV2 {
  return 'createNodes' in plugin || 'createDependencies' in plugin;
}

export function isBuildscalePluginV1(
  plugin: BuildscalePlugin | LoadedBuildscalePlugin
): plugin is BuildscalePluginV1 {
  return 'processProjectGraph' in plugin || 'projectFilePatterns' in plugin;
}

export function normalizeBuildscalePlugin(plugin: BuildscalePlugin): NormalizedPlugin {
  if (isBuildscalePluginV2(plugin)) {
    return plugin;
  }
  if (isBuildscalePluginV1(plugin) && plugin.projectFilePatterns) {
    return {
      ...plugin,
      createNodes: [
