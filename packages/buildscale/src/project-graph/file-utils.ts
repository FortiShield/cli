...
import { getDefaultPluginsSync } from '../utils/buildscale-plugin.deprecated';
  ...
  return files.map((f) => {
    ...
    return {
      ...
      getChanges: (): Change[] => {
        ...
        if (buildscaleArgs.files && buildscaleArgs.files.includes(f)) {
          return [new WholeFileChange()];
        }
        switch (ext) {
          case '.json':
            try {
              const atBase = readFileAtRevision(f, buildscaleArgs.base);
              const atHead = readFileAtRevision(f, buildscaleArgs.head);
...
export function readWorkspaceConfig(opts: {
  ...
  try {
    ...
    configuration = {
      version: 2,
      projects: getProjectsSyncNoInference(root, buildscaleJson).projects,
    };
  }
...
export { readBuildscaleJson, workspaceLayout } from '../config/configuration';

/**
 * TODO(v19): Remove this function.
 */
function getProjectsSyncNoInference(root: string, buildscaleJson: BuildscaleJsonConfiguration) {
