        ...
        if (totalFailedTasks === 0) {
          linesToRender.push(
            `${LEFT_PAD}${output.colors.cyan(
              ...
              `Buildscale is waiting on ${remainingDependentTasksNotFromInitiatingProject} dependent project tasks before running tasks from`
    ...
    if (totalSuccessfulTasks === totalTasks) {
      ...
      if (totalCachedTasks > 0) {
        pinnedFooterLines.push(
          output.dim(
            `${EOL}Buildscale read the output from the cache instead of running the command for ${totalCachedTasks} out of ${totalTasks} tasks.`
          )
        );
      }
