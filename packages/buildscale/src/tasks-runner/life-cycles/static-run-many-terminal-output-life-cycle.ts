    ...
    if (this.failedTasks.length === 0) {
      ...
      const bodyLines =
        this.cachedTasks.length > 0
          ? [
              output.dim(
                `Buildscale read the output from the cache instead of running the command for ${this.cachedTasks.length} out of ${this.tasks.length} tasks.`
              ),
            ]
          : [];
      ...
      if (skippedTasks.length > 0) {
        bodyLines.push(
          output.dim(
            'Tasks not run because their dependencies failed or --buildscale-bail=true:'
          ),
          '',
