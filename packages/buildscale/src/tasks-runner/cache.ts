...
export class Cache {
  ...
  private async assertLocalCacheValidity(task: Task) {
    ...
    if (sourceMachineId && sourceMachineId != (await this.currentMachineId())) {
      if (
        process.env.BUILDSCALE_REJECT_UNKNOWN_LOCAL_CACHE != '0' &&
        process.env.BUILDSCALE_REJECT_UNKNOWN_LOCAL_CACHE != 'false'
      ) {
        const error = [
          ...
          `If your machine ID has changed since the artifact was cached, run "buildscale reset" to fix this issue.`,
          `Read about the error and how to address it here: https://buildscale.github.io/troubleshooting/unknown-local-cache`,
          ``,
        ].join('\n');
