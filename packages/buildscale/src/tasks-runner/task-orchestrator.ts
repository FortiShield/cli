...
export class TaskOrchestrator {
  ...
  private batchEnv = getEnvVariablesForBatchProcess(
    this.options.skipBuildscaleCache,
  ...
  private async executeNextBatchOfTasksUsingTaskSchedule() {
    ...
    const doNotSkipCache =
      this.options.skipBuildscaleCache === false ||
      this.options.skipBuildscaleCache === undefined;
  ...
  private async applyFromCacheOrRunTask(
    ...
    if (results.length === 0) {
      const shouldPrefix =
        streamOutput && process.env.BUILDSCALE_PREFIX_OUTPUT === 'true';
      const targetConfiguration = getTargetConfigurationForTask(
        task,
        this.projectGraph
      );
      if (
        process.env.BUILDSCALE_RUN_COMMANDS_DIRECTLY !== 'false' &&
        targetConfiguration.executor === 'buildscale:run-commands' &&
        !shouldPrefix
      ) {
  ...
  private async runTaskInForkedProcess(
    ...
    try {
      const usePtyFork = process.env.BUILDSCALE_NATIVE_COMMAND_RUNNER !== 'false';
  ...
  private async pipeOutputCapture(task: Task) {
    try {
      if (process.env.BUILDSCALE_NATIVE_COMMAND_RUNNER !== 'false') {
        return true;
      }
  ...
  private shouldCacheTaskResult(task: Task, code: number) {
    return (
      ...
      (process.env.BUILDSCALE_CACHE_FAILURES == 'true' ? true : code === 0)
    );
  }
