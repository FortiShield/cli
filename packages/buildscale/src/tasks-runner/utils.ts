...
export function getCommandAsString(execCommand: string, task: Task) {
  ...
  return [execCommand, 'buildscale', ...args].join(' ').trim();
}
...
export function getExecutorForTask(
  task: Task,
  projectGraph: ProjectGraph
): ExecutorConfig & { isNgCompat: boolean; isBuildscaleExecutor: boolean } {
...
export function shouldStreamOutput(
  ...
  if (process.env.BUILDSCALE_STREAM_OUTPUT === 'true') return true;
...
// TODO: vsavkin remove when buildscale-cloud doesn't depend on it
