...
import * as buildscaleJsonUtils from '../config/buildscale-json';
...
describe('TasksSchedule', () => {
  describe('dependent tasks', () => {
    ...
    beforeEach(() => {
      ...
      jest.spyOn(buildscaleJsonUtils, 'readBuildscaleJson').mockReturnValue({});
    ...
    describe('Without Batch Mode', () => {
      let original;
      beforeEach(() => {
        original = process.env['BUILDSCALE_BATCH_MODE'];
        process.env['BUILDSCALE_BATCH_MODE'] = 'false';
      });

      afterEach(() => {
        process.env['BUILDSCALE_BATCH_MODE'] = original;
      });
