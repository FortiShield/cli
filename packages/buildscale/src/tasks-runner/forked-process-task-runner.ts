...
export class ForkedProcessTaskRunner {
  ...
  private readonly verbose = process.env.BUILDSCALE_VERBOSE_LOGGING === 'true';
  ...
  private forkProcessWithPrefixAndNotTTY(
    ...
    return new Promise<{ code: number; terminalOutput: string }>((res, rej) => {
      try {
        ...
        if (streamOutput) {
          if (process.env.BUILDSCALE_PREFIX_OUTPUT === 'true') {
  ...
  private setupProcessEventListeners() {
    ...
    // When the buildscale process gets a message, it will be sent into the task's process
