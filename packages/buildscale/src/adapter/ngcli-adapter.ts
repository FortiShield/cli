...
import { BUILDSCALE_ERROR, BUILDSCALE_PREFIX } from '../utils/logger';
...
export async function createBuilderContext(
  ...
  require('./compat');
  const fsHost = new Buildscale.copedHostForBuilders(context.root);
  ...
  registry.addSmartDefaultProvider('unparsed', () => {
    // This happens when context.scheduleTarget is used to run a target using buildscale:run-commands
    return [];
  });
  ...
  const builderContext: import('@angular-devkit/architect').BuilderContext = {
    ...
    addTeardown(teardown: () => Promise<void> | void) {
      // No-op as Buildscale doesn't require an implementation of this function
      return;
    },
    reportProgress(...args) {
      // No-op as Buildscale doesn't require an implementation of this function
...
export async function scheduleTarget(
  ...
  const fsHost = new Buildscale.copedHostForBuilders(root);
  const { workspace } = await workspaces.readWorkspace(
    'angular.json',
    workspaces.createWorkspaceHost(fsHost)
  );
...
function createNodeModulesEngineHost(
  ...
  class Buildscale.odeModulesEngineHost extends NodeModulesEngineHost {
    constructor() {
      super(resolvePaths);
    }
    ...
    override _resolveCollectionPath(name: string, requester?: string): string {
      ...
      if (name.endsWith('.json')) {
        ...
        if (!schematics && !generators) {
          throw new Error(
            `The "${name}" package does not support Buildscale generators or Angular Devkit schematics.`
          );
        }
  ...
  return new Buildscale.odeModulesEngineHost();
}
...
export class Buildscale.copedHost extends virtualFs.ScopedHost<any> {
  ...
  protected readMergedWorkspaceConfiguration() {
    return zip(
      ...
      this.readJson<BuildscaleJsonConfiguration>('buildscale.json')
    ).pipe(
      concatMap(([graph, angularJson, buildscaleJson]) => {
        const workspaceConfig = (angularJson || { projects: {} }) as any;
        workspaceConfig.cli ??= buildscaleJson.cli;
        workspaceConfig.schematics ??= buildscaleJson.generators;
 ...
 */
export class Buildscale.copedHostForBuilders extends Buildscale.copedHost {
  protected readMergedWorkspaceConfiguration() {
    return zip(
      from(createProjectGraphAsync()),
      this.readExistingAngularJson(),
      ...
      map(([graph, angularJson, buildscaleJson]) => {
        const workspaceConfig = (angularJson || { projects: {} }) as any;
        workspaceConfig.cli ??= buildscaleJson.cli;
        workspaceConfig.schematics ??= buildscaleJson.generators;
 ...
 */
export class Buildscale.copeHostUsedForWrappedSchematics extends Buildscale.copedHost {
...
export async function generate(
  ...
  const logger = getLogger(verbose);
  const fsHost = new Buildscale.copeHostUsedForWrappedSchematics(
    root,
    new FsTree(
      root,
      verbose,
...
function resolveMigrationsCollection(name: string): string {
  ...
  if (extname(name)) {
    ...
    const { path: packageJsonPath, packageJson } = readModulePackageJson(
      name,
      getBuildscaleRequirePaths(process.cwd())
    );

    let pkgJsonSchematics =
      packageJson['buildscale-migrations'] ?? packageJson['ng-update'];
...
/**
 * If you have an Buildscale Devkit generator invoking the wrapped Angular Devkit schematic,
 * and you don't want the Angular Devkit schematic to run, you can mock it up using this function.
 *
 * Unfortunately, there are some edge cases in the Buildscale.Angular devkit integration that
...
export function wrapAngularDevkitSchematic(
  ...
  // were written with Buildscale in mind, and may care about tags.
  require('./compat');
  ...
  return async (
    ...
    const recorder = (
      ...
      if (event.kind === 'error') {
        ...
        // Apply special handling for the angular.json file, but only when in an Buildscale workspace
    ...
    const fsHost = new Buildscale.copeHostUsedForWrappedSchematics(host.root, host);
...
async function getWrappedWorkspaceNodeModulesArchitectHost(
  ...
  class WrappedWorkspaceNodeModulesArchitectHost extends AngularWorkspaceNodeModulesArchitectHost {
    ...
    private readExecutorsJson(nodeModule: string, builder: string) {
      ...
      if (!executorsFile) {
        throw new Error(
          `The "${nodeModule}" package does not support Buildscale executors or Angular Devkit Builders.`
        );
      }
