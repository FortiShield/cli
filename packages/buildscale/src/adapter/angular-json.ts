...
export const BUILDSCALE_ANGULAR_JSON_PLUGIN_NAME = 'buildscale-angular-json-plugin';

export const Buildscale.ngularJsonPlugin: BuildscalePluginV2 = {
  name: BUILDSCALE_ANGULAR_JSON_PLUGIN_NAME,
  createNodes: [
    'angular.json',
    (f, _, ctx) => ({
...
export default Buildscale.ngularJsonPlugin;
...
export function shouldMergeAngularProjects(
  ...
  if (
    ...
    // e.g. when invoked from `packages/devkit/src/utils/convert-buildscale-executor.ts`
    (includeProjectsFromAngularJson ||
      // Or if a workspace has `@nrwl/angular`/`@buildscale/angular` installed then projects from `angular.json` to be considered by Buildscale.
...
export function isAngularPluginInstalled() {
  try {
    ...
    require.resolve('@buildscale/angular');
    return true;
  } catch {
    try {
