...
export const allowedWorkspaceExtensions = [
  ...
  'installation',
  'release',
  'buildscaleCloudAccessToken',
  'buildscaleCloudUrl',
  'buildscaleCloudEncryptionKey',
  'parallel',
  'cacheDirectory',
  'useDaemonProcess',
  'useInferencePlugins',
    ...
    (originalReadJsonWorkspace) => async (path, host, options) => {
      const modifiedOptions = {
        ...
        allowedWorkspaceExtensions: allowedWorkspaceExtensions as CheckHasKeys<
          ...
          keyof BuildscaleJsonConfiguration
        >,
      };
      try {
        ...
        // or other things that are different between NgCLI and Buildscale config files.
        const projectGraph = await createProjectGraphAsync();
        const buildscaleJson = readBuildscaleJson();

        // Construct old workspace.json format from project graph
        const w: ProjectsConfigurations & BuildscaleJsonConfiguration = {
          ...buildscaleJson,
