...
export class InProcessTaskHasher implements TaskHasher {
  ...
  private useNativeTaskHasher = process.env.BUILDSCALE_NATIVE_TASK_HASHER !== 'false';

  constructor(
    private readonly projectFileMap: ProjectFileMap,
    private readonly allWorkspaceFiles: FileData[],
    private readonly projectGraph: ProjectGraph,
    private readonly buildscaleJson: BuildscaleJsonConfiguration,
    private readonly externalRustReferences: Buildscale.orkspaceFilesExternals | null,
    ...
    if (process.env.BUILDSCALE_CLOUD_ENCRYPTION_KEY) {
      legacyRuntimeInputs.push({ env: 'BUILDSCALE_CLOUD_ENCRYPTION_KEY' });
    }
...
export function getTargetInputs(
  ...
  const namedInputs = getNamedInputs(buildscaleJson, projectNode);

  const targetData = projectNode.data.targets[target];
  const targetDefaults = (buildscaleJson.targetDefaults || {})[target];
...
export function getInputs(
  ...
  const targetDefaults = (buildscaleJson.targetDefaults || {})[task.target.target];
