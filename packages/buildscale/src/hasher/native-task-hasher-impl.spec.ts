...
describe('native task hasher', () => {
  ...
  beforeEach(async () => {
    ...
    await tempFs.createFiles({
      ...
      'buildscale.json': JSON.stringify(buildscaleJson),
    });
  });
      ...
      [
        {
          "details": {
            ...
            "{workspaceRoot}/.buildscaleignore": "3244421341483603138",
            "{workspaceRoot}/buildscale.json": "5219582320960288192",
          },
  ...
  it('should plan non-default filesets', async () => {
    let buildscaleJsonModified = {
      namedInputs: {
        prod: ['!{projectRoot}/**/*.spec.ts'],
      },
    } as any;
    tempFs.writeFile('buildscale.json', JSON.stringify(buildscaleJsonModified));
    ...
    expect(hash).toMatchInlineSnapshot(`
      {
        "details": {
          ...
          "{workspaceRoot}/.gitignore": "3244421341483603138",
          "{workspaceRoot}/.buildscaleignore": "3244421341483603138",
          "{workspaceRoot}/buildscale.json": "4641558175996703359",
        },
  ...
  it('should make a plan with multiple filesets of a project', async () => {
    ...
    tempFs.writeFile('buildscale.json', JSON.stringify(buildscaleJson));
    const workspaceFiles = await retrieveWorkspaceFiles(tempFs.tempDir, {
      'libs/parent': 'parent',
    });
    let builder = new ProjectGraphBuilder(
      ...
      [
        {
          "details": {
            ...
            "{workspaceRoot}/buildscale.json": "10897751101872977225",
          },
   ...
   *
   *
   * gather the project graph + task graph with `buildscale run project:target --graph=graph.json`
   * gather the file-map.json from `.buildscale/cache/file-map.json`
   * gather the buildscale.json file
   */
  // it('should test client workspaces', async () => {
  //   let buildscaleJson = require('buildscale.json');
