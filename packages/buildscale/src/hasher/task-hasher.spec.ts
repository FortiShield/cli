...
describe('TaskHasher', () => {
  process.env.BUILDSCALE_NATIVE_TASK_HASHER = 'false';
  const packageJson = {
    name: 'nrwl',
  };
  ...
  const tsConfigBaseJson = JSON.stringify({
    compilerOptions: {
      paths: {
        '@buildscale/parent': ['libs/parent/src/index.ts'],
        '@buildscale/child': ['libs/child/src/index.ts'],
      },
    },
  });
  const allWorkspaceFiles = [
    { file: 'yarn.lock', hash: 'yarn.lock.hash' },
    { file: 'buildscale.json', hash: 'buildscale.json.hash' },
  ...
  it('should use targetDefaults from buildscale.json', async () => {
    const hasher = new InProcessTaskHasher(
      {
        parent: [
          { file: 'libs/parent/filea.ts', hash: 'a.hash' },
        ...
        nodes: {
          parent: {
            ...
            data: {
              root: 'libs/parent',
              targets: {
                build: { executor: 'buildscale:run-commands' },
              },
            },
          },
          child: {
            ...
            data: {
              root: 'libs/child',
              targets: { build: { executor: 'buildscale:run-commands' } },
            },
          },
        },
        dependencies: {
  ...
  it('should throw an error when failed to execute runtimeCacheInputs', async () => {
    ...
    try {
      ...
      expect(e.message).toContain('Buildscale failed to execute');
  ...
  describe('hashTarget', () => {
    it('should hash executor dependencies of @buildscale packages', async () => {
      const hasher = new InProcessTaskHasher(
        {},
        allWorkspaceFiles,
        {
          nodes: {
            app: {
              ...
              data: {
                root: 'apps/app',
                targets: { build: { executor: '@buildscale/webpack:webpack' } },
              },
            },
          },
          externalNodes: {
            'npm:@buildscale/webpack': {
              name: 'npm:@buildscale/webpack',
              type: 'npm',
              data: {
                packageName: '@buildscale/webpack',
                version: '16.0.0',
              },
            },
          },
      ...
      const hasher = new InProcessTaskHasher(
        ...
        allWorkspaceFiles,
        {
          ...
          externalNodes: {
            ...
            'npm:@buildscale/devkit': {
              name: 'npm:@buildscale/devkit',
              type: 'npm',
              data: {
                packageName: '@buildscale/devkit',
                version: '16.0.0',
                hash: '$buildscale/devkit16$',
              },
            },
            'npm:buildscale': {
              name: 'npm:buildscale',
          ...
          dependencies: {
            'npm:@buildscale/webpack': [
              {
                source: 'npm:@buildscale/webpack',
                target: 'npm:@buildscale/devkit',
                type: DependencyType.static,
              },
              {
                source: 'npm:@buildscale/webpack',
                target: 'npm:buildscale',
                type: DependencyType.static,
              },
              {
                source: 'npm:@buildscale/webpack',
    ...
    it('should not hash when buildscale:run-commands executor', async () => {
      const hasher = new InProcessTaskHasher(
        {},
        [],
        {
    ...
    it('should use externalDependencies to override buildscale:run-commands', async () => {
      const hasher = new InProcessTaskHasher(
        {},
        allWorkspaceFiles,
        {
