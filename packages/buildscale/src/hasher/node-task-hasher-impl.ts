...
export class NodeTaskHasherImpl implements TaskHasherImpl {
  ...
  private async hashNamedInputForDependencies(
    ...
    const namedInputs = {
      ...
      ...this.buildscaleJson.namedInputs,
  ...
  private hashTarget(
    ...
    if (
      ...
      target.executor.startsWith(`@buildscale/`)
    ) {
    ...
    if (invalidFilesetNoPrefix) {
      throw new Error(
        [
          ...
          `If "${invalidFilesetNoPrefix}" is a named input, make sure it is defined in, for instance, buildscale.json.`,
        ].join('\n')
      );
    }
        ...
        exec(
          ...
          (err, stdout, stderr) => {
            if (err) {
              rej(
                new Error(
                  `Buildscale failed to execute {runtime: '${runtime}'}. ${err}.`
                )
              );
            } else {
